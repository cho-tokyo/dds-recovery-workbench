...
use dds_case_manager::{generate_business_diagnostic_docx, Case, CaseId, CaseStorage};
...
/// Chunk 24d-4-2: 業務診断書 DOCX (業務管理用 + お客様用の 2 セクション構造) を
/// 自動生成。CLI からの `--verbose` 表示は DOCX に置き換えたため削除。
fn finalize_diagnose(
    ...
    // Chunk 24d-4-2: 詳細な業務説明 (5 セクション) は CLI verbose ではなく
    // 業務診断書 DOCX (業務管理用 + お客様用の 2 セクション構造) で提供する。
    ...
    let docx_generated = match generate_business_diagnostic_docx(case, &docx_path) {
        Ok(_) => {
            ...
            println!();
            println!("  ※ 業務管理用セクション (営業の内部確認・見積根拠) と");
            println!("    お客様用セクション (口頭説明の参考、専門用語ゼロ) の 2 構造です。");
            println!("  ※ お客様への詳しい説明は『業務診断書.docx』の");
            println!("    「お客様用」セクションをそのまま CRM へ貼り付け / Word で表示できます。");
            true
        }
        Err(e) => {
            eprintln!();
