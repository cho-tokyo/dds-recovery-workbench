...
fn ntfs_healthy_small_enumerates_all_records_and_finds_30_user_files() {
    ...
    assert!(
        volume.total_records() > 23,
        "MFT should have system records 0-23 plus users"
    );
    ...
    assert!(
        user_file_count >= 30,
        "Expected >=30 user files in healthy fixture, got {}",
        user_file_count
    );
}
...
fn ntfs_with_deletions_finds_5_deleted_user_files() {
    ...
    assert!(
        deleted >= 5,
        "Expected >=5 deleted user files, got {}",
        deleted
    );
}
...
fn product_demo_with_volume_api() {
    ...
    println!(
        "Per-record parse errors:    {} (tolerated, iteration continued)\n",
        parse_errors
    );
