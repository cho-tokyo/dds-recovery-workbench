...
fn all_user_files_in_healthy_image_have_resident_data() {
    ...
    for idx in 0..256 {
        ...
        assert!(
            stream.content.runlist_bytes().is_none(),
            "expected resident for {}, got non-resident",
            name.filename
        );
        assert!(
            matches!(stream.content, DataContent::Resident { .. }),
            "expected DataContent::Resident for {}",
            name.filename
        );
    }
    assert!(
        user_file_count >= 30,
        "expected >=30 user files, got {}",
        user_file_count
    );
}
...
fn mft_entry_zero_has_non_resident_data_with_parseable_runlist() {
    ...
    let main = streams
        ...
        .expect("main $DATA on $MFT");
    // 書籍 Chapter 13 記載: $MFT 自身の $DATA は非常駐
    assert!(
        main.content.is_non_resident(),
        "$MFT main $DATA must be non-resident"
    );
    let runlist_bytes = main
        .content
        .runlist_bytes()
        .expect("runlist bytes available");
    assert!(
        !runlist_bytes.is_empty(),
        "runlist must have at least 1 byte (terminator)"
    );
    let runs = parse_runlist(runlist_bytes).expect("$MFT runlist parses cleanly");
    assert!(
        !runs.is_empty(),
        "$MFT must occupy at least one cluster run"
    );
