...
pub enum AttributeType {
    StandardInformation, AttributeList, FileName, ObjectId, SecurityDescriptor,
    VolumeName, VolumeInformation, Data, IndexRoot, IndexAllocation, Bitmap,
    ReparsePoint, EaInformation, Ea, LoggedUtilityStream,
    /// 未知の属性タイプ（エラーにせず保持）。
    Unknown(u32),
    /// 0xFFFFFFFF 終端マーカー。
    End,
...
pub struct AttributeCommonHeader {
    pub attribute_type: AttributeType, pub length: u32, pub non_resident: bool,
    pub name_length: u8, pub name_offset: u16, pub flags: u16, pub attribute_id: u16,
}
/// 常駐属性固有ヘッダ（offset 0x10〜）。関連 FR: FR-LIVE-01。
#[derive(Debug, Clone)]
#[allow(missing_docs)]
pub struct ResidentInfo { pub content_size: u32, pub content_offset: u16, pub indexed: bool }
/// 非常駐属性固有ヘッダ（offset 0x10〜0x40）。関連 FR: FR-LIVE-01。
#[derive(Debug, Clone)]
#[allow(missing_docs)]
pub struct NonResidentInfo {
    pub starting_vcn: u64, pub last_vcn: u64, pub runlist_offset: u16,
    pub compression_unit_size: u16, pub allocated_size: u64,
    pub real_size: u64, pub initialized_size: u64,
}
/// 単一属性ヘッダ（常駐/非常駐/終端）。関連 FR: FR-LIVE-01, FR-LIVE-06。
#[derive(Debug, Clone)]
#[allow(missing_docs)]
pub enum AttributeHeader {
    Resident { common: AttributeCommonHeader, resident: ResidentInfo },
    NonResident { common: AttributeCommonHeader, non_resident: NonResidentInfo },
    /// 0xFFFFFFFF 終端マーカー（属性巡回はここで停止）。
    End,
}
impl AttributeHeader {
    /// 共通ヘッダへのアクセサ。`End` は `None`。
    pub fn common(&self) -> Option<&AttributeCommonHeader> { match self {
        Self::Resident { common, .. } | Self::NonResident { common, .. } => Some(common),
        Self::End => None } }
    /// 属性総バイト長（次属性までのオフセット）。`End` なら 0。
    pub fn length(&self) -> u32 { self.common().map(|c| c.length).unwrap_or(0) }
    /// 属性タイプ。`End` なら `AttributeType::End`。
    pub fn attribute_type(&self) -> AttributeType {
        self.common().map(|c| c.attribute_type).unwrap_or(AttributeType::End) }
    /// 終端マーカーか。
...
pub fn parse_attribute_header(bytes: &[u8]) -> Result<AttributeHeader, AttributeError> {
    let too_small = |need| AttributeError::BufferTooSmall { got: bytes.len(), need };
    if bytes.len() < 4 { return Err(too_small(4)); }
    let type_id = u32::from_le_bytes(bytes[0..4].try_into().expect("len 4"));
    if type_id == END_MARKER { return Ok(AttributeHeader::End); }
    if bytes.len() < MIN_COMMON { return Err(too_small(MIN_COMMON)); }
    ...
    if length == 0 { return Err(AttributeError::InvalidLength { length }); }
    let non_resident = match bytes[0x08] { 0 => false, 1 => true,
        other => return Err(AttributeError::InvalidNonResidentFlag { got: other }) };
    let common = AttributeCommonHeader {
        attribute_type: AttributeType::from_raw(type_id), length, non_resident,
        name_length: bytes[0x09], name_offset: u16le(0x0A),
        flags: u16le(0x0C), attribute_id: u16le(0x0E),
    };
    let need = if non_resident { MIN_NONRESIDENT } else { MIN_RESIDENT };
    if bytes.len() < need { return Err(too_small(need)); }
    if non_resident {
        Ok(AttributeHeader::NonResident { common, non_resident: NonResidentInfo {
            starting_vcn: u64le(0x10), last_vcn: u64le(0x18), runlist_offset: u16le(0x20),
            compression_unit_size: u16le(0x22), allocated_size: u64le(0x28),
            real_size: u64le(0x30), initialized_size: u64le(0x38),
        }})
    } else { Ok(AttributeHeader::Resident { common, resident: ResidentInfo {
        content_size: u32le(0x10), content_offset: u16le(0x14), indexed: bytes[0x16] != 0,
    }}) }
}
...
mod tests {
    ...
    fn br(type_id: u32, length: u32) -> Vec<u8> {
        ...
        b[0..4].copy_from_slice(&type_id.to_le_bytes()); b[4..8].copy_from_slice(&length.to_le_bytes());
        b[0x0A..0x0C].copy_from_slice(&0x18u16.to_le_bytes());
        b[0x0C..0x0E].copy_from_slice(&1u16.to_le_bytes()); b[0x0E..0x10].copy_from_slice(&7u16.to_le_bytes());
    ...
    fn bnr(type_id: u32, real_size: u64) -> Vec<u8> {
        ...
        b[0..4].copy_from_slice(&type_id.to_le_bytes()); b[4..8].copy_from_slice(&0x50u32.to_le_bytes());
        b[0x08] = 1; b[0x18..0x20].copy_from_slice(&3u64.to_le_bytes());
        b[0x20..0x22].copy_from_slice(&0x40u16.to_le_bytes()); b[0x28..0x30].copy_from_slice(&16384u64.to_le_bytes());
        b[0x30..0x38].copy_from_slice(&real_size.to_le_bytes()); b
    }
    #[test]
    fn attribute_type_from_raw_roundtrip_main_types() {
        for (raw, exp) in [(0x10u32, AttributeType::StandardInformation),
            (0x30, AttributeType::FileName), (0x80, AttributeType::Data),
            (0xB0, AttributeType::Bitmap), (0x100, AttributeType::LoggedUtilityStream)] {
    ...
    fn parses_resident_header_all_fields() {
        ...
        assert_eq!((c.attribute_type, c.length, c.non_resident, c.flags, c.attribute_id),
            (AttributeType::FileName, 0x60, false, 1, 7));
        if let AttributeHeader::Resident { resident: r, .. } = h {
            assert_eq!((r.content_size, r.content_offset, r.indexed), (0x48, 0x18, true));
    ...
    fn parses_nonresident_header_all_fields() {
        ...
        if let AttributeHeader::NonResident { non_resident: nr, .. } = h {
            assert_eq!((nr.runlist_offset, nr.real_size, nr.allocated_size, nr.last_vcn),
                (0x40, 12345, 16384, 3));
    ...
    fn end_marker_returned_immediately() {
        ...
        assert!(h.is_end()); assert_eq!(h.length(), 0);
        assert_eq!(h.attribute_type(), AttributeType::End);
        assert!(h.common().is_none());
    }
    #[test]
    fn buffer_too_small_rejected() {
        assert_eq!(parse_attribute_header(&[0u8; 2]).unwrap_err(),
            AttributeError::BufferTooSmall { got: 2, need: 4 });
        assert_eq!(parse_attribute_header(&[0x10u8, 0, 0, 0, 1, 0, 0, 0]).unwrap_err(),
            AttributeError::BufferTooSmall { got: 8, need: MIN_COMMON });
    }
    #[test]
    fn invalid_non_resident_flag_rejected() {
        let mut buf = br(0x10, 0x20); buf[0x08] = 2;
    ...
    fn zero_length_rejected_prevents_infinite_loop() {
        let mut buf = br(0x10, 0x20); buf[4..8].copy_from_slice(&0u32.to_le_bytes());
        assert!(matches!(parse_attribute_header(&buf).unwrap_err(),
