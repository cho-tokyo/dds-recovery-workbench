...
/// 関連 FR: FR-LIVE-01, FR-LIVE-04。
pub fn apply_fixup(bytes: &mut [u8], usa_offset: u16, usa_size: u16, sector_size: u16)
    -> Result<(), FixupError>
{
    if usa_size == 0 { return Err(FixupError::InvalidUsaSize { size: usa_size }); }
    let (usa_off, usa_bytes) = (usa_offset as usize, (usa_size as usize) * 2);
    let need = usa_off.checked_add(usa_bytes)
        .ok_or(FixupError::InvalidUsaOffset { offset: usa_offset })?;
    if need > bytes.len() { return Err(FixupError::BufferTooSmall { got: bytes.len(), need }); }
    let usn = u16::from_le_bytes([bytes[usa_off], bytes[usa_off + 1]]);
    let (sectors, ss) = ((usa_size as usize) - 1, sector_size as usize);
    if ss < 2 { return Err(FixupError::InvalidUsaSize { size: usa_size }); }
    let span = sectors.checked_mul(ss).ok_or(FixupError::InvalidUsaSize { size: usa_size })?;
    if span > bytes.len() { return Err(FixupError::BufferTooSmall { got: bytes.len(), need: span }); }
    for i in 0..sectors {
        let pos = ss * (i + 1) - 2;
        let got = u16::from_le_bytes([bytes[pos], bytes[pos + 1]]);
        if got != usn { return Err(FixupError::FixupMismatch { sector: i, expected: usn, got }); }
...
mod tests {
    ...
    fn build(usn: u16, fx0: u16, fx1: u16, tail1: u16) -> Vec<u8> {
        ...
        b[0x3FE..0x400].copy_from_slice(&tail1.to_le_bytes()); b
    }
    ...
    fn apply_fixup_propagates_mismatch_error() {
        ...
        assert_eq!(apply_fixup(&mut buf, 0x30, 3, 512).unwrap_err(),
            FixupError::FixupMismatch { sector: 1, expected: 0x1234, got: 0x9999 });
    }
    /// `usa_size == 0` は無効値として弾く（無限ループ・除算エラー防止）。
    #[test]
    fn apply_fixup_rejects_zero_usa_size() {
        let mut buf = vec![0u8; 1024];
        assert_eq!(apply_fixup(&mut buf, 0x30, 0, 512).unwrap_err(),
