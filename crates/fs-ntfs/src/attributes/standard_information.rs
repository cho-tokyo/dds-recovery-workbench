...
#[derive(Debug, Clone, Copy, PartialEq, Eq)] #[allow(missing_docs)] pub struct FileTime(pub u64);
impl FileTime {
    /// `chrono::DateTime<Utc>` に変換する。範囲外（i64 オーバーフロー等）は `None`。
    pub fn to_datetime(&self) -> Option<DateTime<Utc>> {
        let total = i64::try_from(self.0).ok()?;
        let secs = total.checked_div(10_000_000)?.checked_sub(FILETIME_EPOCH_DIFF_SECS)?;
        let nanos = ((total % 10_000_000) as u32).checked_mul(100)?;
        DateTime::from_timestamp(secs, nanos)
    }
}
/// DOS ファイル属性ビットフラグ。各 `is_*` で個別ビットを判定する。関連 FR: FR-LIVE-06。
#[derive(Debug, Clone, Copy)] #[allow(missing_docs)] pub struct FileAttributes(pub u32);
...
#[derive(Debug, Clone)] #[allow(missing_docs)]
pub struct StandardInformation {
    pub created: FileTime, pub modified: FileTime,
    pub mft_modified: FileTime, pub accessed: FileTime,
    pub file_attributes: FileAttributes,
    pub max_versions: u32, pub version_number: u32, pub class_id: u32,
    pub owner_id: Option<u32>, pub security_id: Option<u32>,
    pub quota_charged: Option<u64>, pub usn: Option<u64>,
}
/// `parse_standard_information` が返すエラー型。
#[derive(Debug, Error, PartialEq, Eq)] #[allow(missing_docs)]
pub enum SiError {
...
pub fn parse_standard_information(bytes: &[u8]) -> Result<StandardInformation, SiError> {
    if bytes.len() < MIN_SIZE { return Err(SiError::BufferTooSmall { got: bytes.len() }); }
    ...
    Ok(StandardInformation {
        created: FileTime(u64le(0x00)), modified: FileTime(u64le(0x08)),
        mft_modified: FileTime(u64le(0x10)), accessed: FileTime(u64le(0x18)),
        file_attributes: FileAttributes(u32le(0x20)),
        max_versions: u32le(0x24), version_number: u32le(0x28), class_id: u32le(0x2C),
...
mod tests {
    ...
    #[test] fn parses_48_byte_nt_version() {
        ...
        assert!(si.owner_id.is_none() && si.security_id.is_none()
            && si.quota_charged.is_none() && si.usn.is_none());
    }
    #[test] fn parses_72_byte_w2k_extended() {
        let si = parse_standard_information(&build_si(true)).expect("parse");
        assert_eq!((si.owner_id, si.security_id, si.quota_charged, si.usn),
            (Some(111), Some(222), Some(333), Some(444)));
    }
    #[test] fn rejects_buffer_smaller_than_48_bytes() {
        assert_eq!(parse_standard_information(&[0u8; 47]).unwrap_err(),
            SiError::BufferTooSmall { got: 47 });
    }
    #[test] fn filetime_to_datetime_known_value() {
    ...
    #[test] fn file_attributes_bit_checks() {
        let a = FileAttributes(FileAttributes::READ_ONLY | FileAttributes::HIDDEN
            | FileAttributes::SYSTEM | FileAttributes::ARCHIVE | FileAttributes::COMPRESSED
            | FileAttributes::ENCRYPTED | FileAttributes::DIRECTORY);
