...
impl IndxBlock {
    ...
    pub fn node_body(&self) -> &[u8] {
        &self.data[self.node_header_offset + NODE_HDR..]
    }
}
...
fn parse_index_node_header(bytes: &[u8]) -> Result<IndexNodeHeader, IndexError> {
    ...
    Ok(IndexNodeHeader {
        first_entry_offset: u32le(0),
        end_of_entries_offset: u32le(4),
        end_of_buffer_offset: u32le(8),
        flags: bytes[12],
    })
}
...
pub fn parse_index_root(bytes: &[u8]) -> Result<IndexRoot<'_>, IndexError> {
    ...
    if index_type != FN_TYPE {
        return Err(IndexError::UnsupportedIndexType { got: index_type });
    }
    let node_header = parse_index_node_header(&bytes[STD_HDR..need])?;
    Ok(IndexRoot {
        index_type,
        collation_rule: u32le(4),
        bytes_per_index_record: u32le(8),
        clusters_per_index_record: bytes[12] as i8,
        node_header,
        node_body: &bytes[need..],
    })
}
...
pub fn parse_entries_in_node(node_body: &[u8]) -> Result<Vec<IndexEntry>, IndexError> {
    ...
    loop {
        ...
        let child_raw =
            u64::from_le_bytes(node_body[cursor..cursor + 8].try_into().expect("len 8"));
        ...
        let file_name = if !is_last && fn_len > 0 {
            ...
            if e > cursor + elen {
                return Err(IndexError::EntryLengthExceedsBuffer {
                    length: fn_len,
                    remaining: elen.saturating_sub(ENTRY_HDR),
                });
            }
        ...
        let child_vcn = if has_child && elen >= 8 {
            ...
            Some(u64::from_le_bytes(
                node_body[vo..vo + 8].try_into().expect("len 8"),
            ))
        } else {
            None
        };
...
mod tests {
    ...
    fn build_index_root(items: &[(u64, &str)]) -> Vec<u8> {
        ...
        put32(&mut buf, 0, FN_TYPE);
        put32(&mut buf, 8, 4096);
        buf[12] = 1;
        let eo = (NODE_HDR + entries.len()) as u32;
        put32(&mut buf, STD_HDR, 16);
        put32(&mut buf, STD_HDR + 4, eo);
        put32(&mut buf, STD_HDR + 8, eo);
        buf[STD_HDR + NODE_HDR..].copy_from_slice(&entries);
        buf
    }
    ...
    fn build_indx(vcn: u64, items: &[(u64, &str)]) -> Vec<u8> {
        ...
        put32(&mut buf, INDX_HDR + 4, eo);
        put32(&mut buf, INDX_HDR + 8, eo);
    ...
    #[test]
    fn parse_index_root_minimal_valid_directory() {
    ...
    #[test]
    fn parse_index_root_rejects_non_filename_type() {
        let mut buf = build_index_root(&[(64, "x")]);
        put32(&mut buf, 0, 0x90);
        assert!(matches!(
            parse_index_root(&buf).unwrap_err(),
            IndexError::UnsupportedIndexType { got: 0x90 }
        ));
    }
    #[test]
    fn parse_index_root_buffer_too_small() {
        assert!(matches!(
            parse_index_root(&[0u8; 31]).unwrap_err(),
            IndexError::BufferTooSmall { got: 31, need: 32 }
        ));
    }
    #[test]
    fn parse_indx_block_with_valid_magic_and_fixup() {
    ...
    #[test]
    fn parse_indx_block_rejects_invalid_magic() {
    ...
    #[test]
    fn parse_indx_block_fixup_mismatch_propagates() {
        let mut blk = build_indx(0, &[(64, "x")]);
        let last = blk.len() - 2;
        blk[last..].copy_from_slice(&0xDEADu16.to_le_bytes());
        assert!(matches!(
            parse_indx_block(&blk, 512).unwrap_err(),
            IndexError::Fixup(FixupError::FixupMismatch { sector: 7, .. })
        ));
    }
    #[test]
    fn parse_entries_single_terminal_entry() {
        let mut t = vec![0u8; 16];
        put16(&mut t, 8, 16);
        put32(&mut t, 12, F_LAST);
    ...
    #[test]
    fn parse_entries_multiple_with_filenames() {
        let buf = build_entries(&[(10, "a.txt"), (11, "b.txt"), (12, "c.txt")]);
        let entries = parse_entries_in_node(&buf).expect("ok");
        assert_eq!(entries.len(), 4);
        let names: Vec<_> = entries
            .iter()
            .filter_map(|e| e.file_name.as_ref().map(|f| f.filename.as_str()))
            .collect();
        assert_eq!(names, vec!["a.txt", "b.txt", "c.txt"]);
        assert_eq!(entries[0].child_ref.entry_number, 10);
    }
    #[test]
    fn parse_entries_zero_length_returns_error() {
        assert!(matches!(
            parse_entries_in_node(&[0u8; 16]).unwrap_err(),
            IndexError::EntryLengthZero
        ));
    }
    #[test]
    fn parse_entries_length_exceeds_buffer_returns_error() {
        let mut buf = vec![0u8; 16];
        put16(&mut buf, 8, 64);
        assert!(matches!(
            parse_entries_in_node(&buf).unwrap_err(),
            IndexError::EntryLengthExceedsBuffer { length: 64, .. }
        ));
    }
    #[test]
    fn parse_entries_with_child_node_vcn_extracted() {
