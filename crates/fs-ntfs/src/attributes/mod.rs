...
pub use data::{
    extract_all_data_streams, extract_main_data_stream, parse_data_stream, DataContent, DataError,
    DataStream,
};
pub use file_name::{
...
pub use index::{
    parse_entries_in_node, parse_index_root, parse_indx_block, IndexEntry, IndexError,
    IndexNodeHeader, IndexRoot, IndxBlock,
};
        ...
        match parse_attribute_header(&self.entry_data[self.cursor..]) {
            ...
            Ok(header) => {
                ...
                if length == 0 || self.cursor + length > self.entry_data.len() {
                    self.done = true;
                    return Some(Err(AttributeError::InvalidLength {
...
pub fn find_attribute<'a>(
    ...
    AttributeIterator::new(entry_data, first_attribute_offset)
        ...
        .find(|a| a.header.attribute_type() == target_type)
}
...
mod tests {
    ...
    #[test]
    fn iterator_empty_on_end_marker() {
        let d = entry(&[]);
        let mut it = AttributeIterator::new(&d, 0);
        assert!(it.next().is_none());
        assert!(it.next().is_none());
    }
    #[test]
    fn iterator_yields_single_attribute_then_end() {
    ...
    #[test]
    fn iterator_yields_multiple_attributes() {
        ...
        assert_eq!(
            types,
            vec![AttributeType::StandardInformation, AttributeType::FileName]
        );
    }
    #[test]
    fn find_attribute_finds_existing_type() {
        let d = entry(&[resident(0x10, 0x60), resident(0x30, 0x70)]);
        let f = find_attribute(&d, 0, AttributeType::FileName).expect("found");
        assert_eq!(
            (f.header.attribute_type(), f.offset_in_entry),
            (AttributeType::FileName, 0x60)
        );
    }
    #[test]
    fn find_attribute_returns_none_for_missing_type() {
        let d = entry(&[resident(0x10, 0x60)]);
