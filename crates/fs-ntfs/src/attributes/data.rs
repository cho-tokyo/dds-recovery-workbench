...
fn extract_attribute_name(raw: &[u8], h: &AttributeCommonHeader) -> Result<String, DataError> {
    ...
    let end = off
        .checked_add((h.name_length as usize) * 2)
        .ok_or(DataError::InvalidStreamName)?;
    if end > raw.len() {
        return Err(DataError::InvalidStreamName);
    }
    let u: Vec<u16> = raw[off..end]
        .chunks_exact(2)
...
pub fn parse_data_stream<'a>(
    ...
    if c.attribute_type != AttributeType::Data {
    ...
    let content = match header {
        AttributeHeader::Resident { resident: r, .. } => {
            ...
            if s > attr_raw.len() {
                return Err(DataError::InvalidContentOffset {
                    offset: r.content_offset,
                });
            }
            let e = s
                .checked_add(r.content_size as usize)
        ...
        AttributeHeader::NonResident {
            ...
            real_size: n.real_size,
            allocated_size: n.allocated_size,
            starting_vcn: n.starting_vcn,
            last_vcn: n.last_vcn,
            runlist_offset_in_attr: n.runlist_offset as usize,
...
pub fn extract_all_data_streams<'a>(
    ...
    AttributeIterator::new(entry_data, first_attribute_offset)
        ...
        .filter_map(|a| parse_data_stream(a.raw, &a.header).ok())
        .collect()
}
...
mod tests {
    ...
    fn br(name: &str, content: &[u8], flags: u16) -> (Vec<u8>, AttributeHeader) {
        ...
        let total = co as usize + content.len();
        ...
        r[0x09] = u.len() as u8;
        r[0x0A..0x0C].copy_from_slice(&no.to_le_bytes());
        ...
        put_name(&mut r, no as usize, &u);
        r[co as usize..].copy_from_slice(content);
        (
            r,
            AttributeHeader::Resident {
                common: ch(u.len() as u8, no, flags, total as u32, false),
    ...
    fn bnr(name: &str, real_size: u64) -> (Vec<u8>, AttributeHeader) {
        ...
        let total = ro as usize + 8;
        let mut r = vec![0u8; total];
        r[0..4].copy_from_slice(&AttributeType::Data.to_raw().to_le_bytes());
        r[4..8].copy_from_slice(&(total as u32).to_le_bytes());
        r[0x08] = 1;
        r[0x09] = u.len() as u8;
        r[0x0A..0x0C].copy_from_slice(&0x40u16.to_le_bytes());
        put_name(&mut r, 0x40, &u);
        (
            r,
            AttributeHeader::NonResident {
                common: ch(u.len() as u8, 0x40, 0, total as u32, true),
    ...
    #[test]
    fn resident_data_content_extraction() {
    ...
    #[test]
    fn empty_unnamed_and_named_decoded() {
        let (r0, h0) = br("", b"", 0);
        let (r6, h6) = br("", b"abcdef", 0);
        let (rs, hs) = br("secret", b"xyz", 0);
        assert_eq!(parse_data_stream(&r0, &h0).unwrap().content.size(), 0);
        let s6 = parse_data_stream(&r6, &h6).unwrap();
        assert!(s6.name.is_empty());
        assert_eq!(s6.content.size(), 6);
        assert_eq!(parse_data_stream(&rs, &hs).unwrap().name, "secret");
    }
    #[test]
    fn japanese_named_stream_decoded() {
        let (r, h) = br("秘匿データ", b"hidden", 0);
        assert_eq!(parse_data_stream(&r, &h).unwrap().name, "秘匿データ");
    }
    #[test]
    fn data_content_is_resident_check() {
        let (r1, h1) = br("", b"x", 0);
        let (r2, h2) = bnr("", 4096);
        assert!(parse_data_stream(&r1, &h1).unwrap().content.is_resident());
        assert!(parse_data_stream(&r2, &h2)
            .unwrap()
            .content
            .is_non_resident());
    }
    #[test]
    fn non_resident_data_info_extraction() {
        let (r, h) = bnr("", 1_000_000);
        if let DataContent::NonResident {
            real_size,
            last_vcn,
            runlist_offset_in_attr,
            ..
        } = parse_data_stream(&r, &h).unwrap().content
        {
            assert_eq!(
                (real_size, last_vcn, runlist_offset_in_attr),
                (1_000_000, 7, 0x40)
            );
        } else {
    ...
    #[test]
    fn extract_all_and_main_data_streams() {
        let (a, _) = br("", b"main", 0);
        let (b, _) = br("ads1", b"x", 0);
        let (c, _) = br("ads2", b"yz", 0);
        let e = cat(&[&a, &b, &c]);
        let s = extract_all_data_streams(&e, 0);
        assert_eq!(s.len(), 3);
        assert_eq!(
        ...
        if let DataContent::Resident { bytes, .. } = m.content {
            assert_eq!(bytes, b"main");
        } else {
            panic!()
        }
    }
    #[test]
    fn flags_compressed_encrypted_sparse_decoded() {
        let (r, h) = br("", b"x", FLAG_COMPRESSED | FLAG_ENCRYPTED | FLAG_SPARSE);
        let s = parse_data_stream(&r, &h).unwrap();
        assert!(s.is_compressed && s.is_encrypted && s.is_sparse);
    }
    #[test]
    fn non_data_attribute_type_rejected() {
        let mut c = ch(0, 0x18, 0, 0x20, false);
        c.attribute_type = AttributeType::FileName;
        let h = AttributeHeader::Resident {
            common: c,
            resident: ResidentInfo {
                content_size: 0,
                content_offset: 0x18,
    ...
    #[test]
    fn zone_identifier_ads_name_decoded() {
        let (a, _) = br("", b"content", 0);
        let (z, _) = br("Zone.Identifier", b"[ZoneTransfer]\r\nZoneId=3", 0);
        let e = cat(&[&a, &z]);
        let all = extract_all_data_streams(&e, 0);
        assert_eq!(all.len(), 2);
        assert_eq!(
    ...
    #[test]
    fn book_figure_12_4_dual_encrypted_data_streams() {
        let (u, _) = br("", b"plain-bytes", FLAG_ENCRYPTED);
        let (n, _) = br("ADS", b"ads-bytes", FLAG_ENCRYPTED);
        let e = cat(&[&u, &n]);
        let all = extract_all_data_streams(&e, 0);
        assert_eq!(all.len(), 2);
        assert!(all.iter().all(|s| s.is_encrypted));
