...
pub fn parse_runlist(bytes: &[u8]) -> Result<Vec<Run>, RunlistError> {
    ...
    loop {
        ...
        if length_bytes == 0 || length_bytes > 8 || offset_bytes > 8 {
            return Err(RunlistError::InvalidHeaderNibble {
                length_bytes,
                offset_bytes,
            });
        }
        let length = read_unsigned_le(bytes, cursor, length_bytes as usize).map_err(|got| {
            RunlistError::LengthFieldTruncated {
                need: length_bytes as usize,
                got,
            }
        })?;
        ...
        let lcn = if offset_bytes == 0 {
            ...
            let delta = read_signed_le(bytes, cursor, offset_bytes as usize).map_err(|got| {
                RunlistError::OffsetFieldTruncated {
                    need: offset_bytes as usize,
                    got,
                }
            })?;
            ...
            current_lcn = current_lcn
                .checked_add(delta)
                .ok_or(RunlistError::LcnOverflow {
                    previous: current_lcn,
                    delta,
                })?;
            if current_lcn < 0 {
...
fn read_signed_le(bytes: &[u8], offset: usize, count: usize) -> Result<i64, usize> {
    ...
    if count > 0 && bytes[offset + count - 1] & 0x80 != 0 {
        for b in buf.iter_mut().skip(count) {
            *b = 0xFF;
        }
    }
...
mod tests {
    ...
    fn book_chapter13_runlist_example_two_runs() {
        let b = [
            0x32, 0xc0, 0x1e, 0xb5, 0x3a, 0x05, 0x21, 0x70, 0x1b, 0x1f, 0x00,
        ];
    ...
    #[test]
    fn single_run_then_end_marker() {
        assert_eq!(
            parse_runlist(&[0x11, 0x05, 0x0A, 0x00]).unwrap(),
            vec![Run {
                length_clusters: 5,
                lcn: Some(10)
            }]
    ...
    #[test]
    fn empty_runlist_immediate_end() {
        assert_eq!(parse_runlist(&[0x00]).unwrap(), Vec::<Run>::new());
    }
    #[test]
    fn unterminated_runlist_returns_buffer_too_small() {
        assert!(matches!(
            parse_runlist(&[0x11, 0x05, 0x0A]),
            Err(RunlistError::BufferTooSmall { .. })
        ));
    }
    #[test]
    fn sparse_run_offset_bytes_zero() {
    ...
    #[test]
    fn sparse_mixed_with_normal_runs() {
    ...
    #[test]
    fn sign_extension_negative_one_byte_offset() {
        // L=1,O=1,len=5,offset=0xFF → -1 → NegativeLcn
        assert!(matches!(
            parse_runlist(&[0x11, 0x05, 0xFF, 0x00]),
            Err(RunlistError::NegativeLcn { got: -1 })
        ));
    }
    #[test]
    fn sign_extension_three_byte_offset_high_bit_set() {
    ...
    #[test]
    fn invalid_header_nibble_length_zero_with_data() {
        assert!(matches!(
            parse_runlist(&[0xF0, 0xAA]),
            Err(RunlistError::InvalidHeaderNibble {
                length_bytes: 0,
                offset_bytes: 15
            })
        ));
    ...
    #[test]
    fn invalid_header_nibble_offset_over_eight() {
        assert!(matches!(
            parse_runlist(&[0x91, 0x05, 0xAA]),
            Err(RunlistError::InvalidHeaderNibble {
                length_bytes: 1,
                offset_bytes: 9
            })
    ...
    #[test]
    fn length_field_truncated_returns_specific_error() {
        assert!(matches!(
            parse_runlist(&[0x14, 0xAA, 0xBB]),
            Err(RunlistError::LengthFieldTruncated { need: 4, got: 2 })
        ));
    }
    #[test]
    fn negative_lcn_after_subtraction_returns_negative_lcn_error() {
        // 第1run: LCN=100、第2run: delta=-200 (LE: 0x38 0xFF) → LCN=-100
        assert!(matches!(
            parse_runlist(&[0x11, 0x05, 0x64, 0x21, 0x02, 0x38, 0xFF, 0x00]),
            Err(RunlistError::NegativeLcn { got: -100 })
        ));
    }
    #[test]
    fn read_runs_with_mock_reader_assembles_continuous_data() {
        let r = vec![
            Run {
                length_clusters: 2,
                lcn: Some(10),
    ...
    #[test]
    fn read_runs_with_sparse_run_fills_zeros() {
        let r = vec![
            Run {
                length_clusters: 1,
                lcn: Some(10),
    ...
    #[test]
    fn read_runs_with_truncates_to_real_size() {
        let r = vec![Run {
            length_clusters: 2,
            lcn: Some(0),
        }];
    ...
    #[test]
    fn read_runs_with_cluster_size_zero_returns_invalid_cluster_size() {
        let r = vec![Run {
            length_clusters: 1,
            lcn: Some(0),
        }];
        assert!(matches!(
            read_runs_with(&r, 0, 4, |_, _| Ok(vec![])),
