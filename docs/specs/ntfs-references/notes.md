...
## 11. $DATA 属性と ADS（Alternate Data Streams）

書籍 第 12 章「NTFS Analysis」の「$DATA Attribute」セクション（特に 318〜319 ページ、
Figure 12.4）と 第 13 章「NTFS Data Structures」の「$DATA Attribute」セクションを
読み込み、実装観点で必要な要点を **自前の言葉で再構成** したもの。書籍本文の逐語
コピーはなし。原典への言及（章番号・図番号・ページ番号）は事実情報として記載。

### 11.1 ネイティブ構造を持たない属性

属性タイプ 0x80。書籍 364 ページの本文要約: $DATA の中身はバイト列そのものであり、
固定レイアウトのレコード型を持たない。`$STANDARD_INFORMATION` や `$FILE_NAME` の
ようなフィールドテーブルが存在せず、ヘッダの後にはファイル本体のバイトがそのまま
（常駐の場合）または runlist（非常駐の場合）として続く。

実装はヘッダのフラグ・サイズ・名前のみを解釈し、コンテンツ自体は呼び出し側（バリ
データ、ハッシュ計算、復旧パイプライン）に生バイトで委ねる。

### 11.2 無名ストリームと ADS（Alternate Data Streams）

書籍 318 ページの本文要約: 1 つの MFT エントリは複数の $DATA 属性を持てる。最初の
無名 $DATA（`name_length = 0`）がいわゆる「メインストリーム」で、コマンドや
エクスプローラで `file.txt` と表記したときに開かれる本体。それ以外の名前付き
$DATA は ADS（Alternate Data Stream）と呼び、`file.txt:streamname` という構文で
アクセスされる。

実装の `extract_main_data_stream` は無名（`name.is_empty()`）の 1 件を返し、
`extract_all_data_streams` は全 ADS を含むベクタを返す。希望リスト突合やハッシュ
比較は通常メインストリーム優先で行うが、ADS にも本体相当のデータが入る運用例
（ZoneTransfer 等）があるため、両 API を併用できる構造を維持する。

### 11.3 常駐／非常駐の閾値（参考情報）

書籍は「だいたい 700 バイトを超えると非常駐になる確率が高い」と確定的でない
書き方をする。実装は閾値で判定せず、属性ヘッダの non-resident フラグ（0/1）の値
だけで分岐する。Phase 1 では参考情報に留め、診断ヒューリスティクスでも閾値
ベースの判定は使わない。

### 11.4 典型的な ADS 例

書籍 318 ページが触れる ADS の現実例:

- `Zone.Identifier` — Windows がインターネット由来ファイルに付与するゾーン情報
  ADS（MOTW: Microsoft の Zone Identifier 仕様）。INI 形式のテキスト
  （`[ZoneTransfer]` セクションに `ZoneId=3` 等）を保持。ファイルマネージャ上は
  無名ストリームのみ見えるが、ADS 列挙でこの名前が現れる。
- TSK 等のフォレンジックツールは、表示上の便宜のため無名 $DATA に `$DATA` という
  ラベルを付ける慣例がある。実装内部では空文字列の `name` で扱うが、レポート出力
  では UI 層で同じ慣例を採用してもよい（Phase 1 では空文字列のまま）。

### 11.5 暗号化と $LOGGED_UTILITY_STREAM

書籍 319 ページの本文要約: EFS（Encrypting File System）で暗号化されたファイルは、
$DATA 属性に `FLAG_ENCRYPTED`（bit14, 0x4000）が立ち、暗号鍵情報は
$LOGGED_UTILITY_STREAM（属性タイプ 0x100）に格納される。Figure 12.4 の例題では、
1 つの MFT エントリに無名 $DATA と名前付き ADS の両方が並び、両方とも encrypted
フラグがセットされた状態が示されている。

Phase 1 では復号は行わず、`is_encrypted` フラグの保持に留める。診断レポートには
「暗号化ストリームを検出（復号は実装範囲外）」と記録し、復旧パイプラインは生の
暗号化バイト列をそのまま出力する選択肢を持つ。
