...
## 🎯 Chunk 24b: 並列化によるパフォーマンス改善 + 進捗表示 — 完成 🎯（Chunk 24b / 2026-05-26）

### 🎯 Chunk 24b ハイライト（Phase 1.5 業務適用品質の最終形完成 / 4 MB/s → 50-100 MB/s 目標）

**🎯 業務的背景**: 実機ドライラン (2026-05-22) で判明した課題:
- ② ③ **復旧速度 4 MB/s + 進捗見えない** → Producer-Consumer 並列化 + ConsoleProgressReporter で対応
- 目標: 50-100 MB/s（12-25 倍速、100 MB/s 最低ライン）
- 「動いている感」表示で待ち時間中のお客様不安解消

**🎯 設計原則の継承**:
- 「unsafe 追加なし」維持: **Chunk 24a の 2 ブロックから増加なし** (`timestamps.rs:132, 143`)
- crossbeam-channel / num_cpus / sha2 / BufWriter / std::thread::spawn すべて safe API
- **並列化に unsafe 不要を達成**（業界としては注目に値する設計成果）
- ProgressReporter は `Send + Sync` 制約で並列安全
- WorkerPanic 検出と RecoveryError への変換

**🎯 新規ファイル（1）**:
- `crates/recovery/src/progress.rs` (221 行): `ProgressReporter` trait + `ConsoleProgressReporter` + `NoopProgressReporter` + 7 単体テスト
- `Send + Sync` 制約付き trait（並列処理対応 + Tauri 再利用可能設計）
- 5 秒おき stderr 出力、初回即表示、100% 時に最終報告

**🎯 修正ファイル（13）**:
- `Cargo.toml`: workspace deps に `crossbeam-channel = "0.5"` + `num_cpus = "1.16"` 追加
- `crates/recovery/Cargo.toml`: crossbeam-channel / num_cpus workspace deps
- **`crates/recovery/src/engine.rs`**: **`recover_files` シグネチャに `progress: &P` 引数追加**、Producer-Consumer 並列化実装、I/O バッファ 1MB
- `crates/recovery/src/error.rs`: `WorkerPanic` バリアント追加
- `crates/recovery/src/lib.rs`: progress mod + re-export
- `crates/case-manager/Cargo.toml`: num_cpus dev-dep 追加（perf demo 用）
- **`crates/case-manager/src/orchestration.rs`**: `execute_business_recovery` シグネチャに `progress: &P` 追加
- `crates/case-manager/tests/business_flow_integration.rs`: migration + 結合 2 件追加 (`perf_demo_chunk24b_recovery_speed` / `demo_chunk24b_console_progress_output`、両方 `#[ignore]`)
- `crates/recovery/tests/recovery_integration.rs`: migration + 並列結合 2 件追加 (`parallel_recovery_processes_all_files` / `parallel_recovery_progress_called_for_each_file`)
- `crates/recovery/tests/recovery_mixed_formats_integration.rs`: migration
- `crates/recovery/tests/recovery_validation_integration.rs`: migration
- `crates/recovery/tests/recovery_with_reports_integration.rs`: migration
- `crates/workbench-dryrun/src/commands/recover.rs`: `ConsoleProgressReporter` 利用 + 復旧完了後の速度 (MB/s) 表示

**🎯 Producer-Consumer 並列化アーキテクチャ**:

```
[プロデューサ (メインスレッド、1 個)]
  ├ NtfsVolume::iter_files() で列挙 (シリアル、FnMut 制約)
  ├ FileInfo 構築 + match_files でマッチング
  ├ MatchResult<'a> を owned (is_priority, labels, score) に展開
  │   (ライフタイム問題回避)
  ├ volume.read_file_content(file) で content 読出 (シリアル必須)
  ├ progress.report(idx+1, total, &path) で進捗報告
  └ task_tx.send(RecoveryTask)  ← bounded(N*2) で背圧制御

[コンシューマ (ワーカースレッド、N 個、N = num_cpus::get().clamp(1, 4))]
  ├ task_rx.recv()
  ├ build_output_path + find_unique_path (衝突解決)
  ├ write_with_large_buffer (BufWriter::with_capacity(1 MiB))
  ├ apply_timestamps (Windows SetFileTime API、Chunk 24a)
  ├ sha256_hex (CPU バウンド)
  ├ ValidatorRegistry::validate (CPU バウンド、DOCX 等)
  └ result_tx.send(ProcessedOutcome)

[結果収集 (1 個)]
  └ result_rx を drain して Vec<ProcessedOutcome> 構築
```

**🎯 設計上のキーポイント**:
- **背圧制御**: `bounded(worker_count * 2)` でメモリ消費を制限
- **NtfsVolume シリアル制約**: FnMut のため read は必ずメインスレッド
- **MatchResult ライフタイム問題**: owned タプル事前展開でワーカーへ
- **I/O バッファ拡大**: デフォルト 8KB → 1MB（syscall 削減）

**🎯 ワーカー数の決定根拠**:
```rust
let worker_count = num_cpus::get().clamp(1, MAX_WORKER_THREADS);  // MAX = 4
```
- 単一コア PC でも動く（clamp 下限 1）
- 8 コア超でも 4 で打ち止め（I/O 支配で並列化効果飽和）
- 業務 PC 16 コア環境で 4 並列確認

**🎯 ConsoleProgressReporter 出力サンプル（業務メンバー提示用）**:
```
=== Chunk 24b ConsoleProgressReporter Sample Output ===
[復旧中] 1/30 ファイル (3.3%) - 経過 0:00 - 現在: \file_000.txt
[復旧中] 14/30 ファイル (46.7%) - 経過 0:00 - 現在: \file_013.txt
[復旧中] 18/30 ファイル (60.0%) - 経過 0:00 - 現在: \file_017.txt
[復旧中] 22/30 ファイル (73.3%) - 経過 0:00 - 現在: \file_021.txt
[復旧中] 26/30 ファイル (86.7%) - 経過 0:00 - 現在: \file_025.txt
[復旧中] 30/30 ファイル (100.0%) - 経過 0:00 - 現在: \file_029.txt
[復旧中] 30/30 ファイル (100.0%) - 経過 0:00 - 現在: 
=== Sample End ===
```

実機 (1858 ファイル / 4.52 GB) では 5 秒おき + 100% 時に stderr 出力される想定。
...
ワーカー数:   4 (CPU コア数 = 16)
ファイル数:   15 件
データ量:    1355 bytes (0.00 MB)
経過時間:    0.034 秒
速度:        0.0 MB/s

注: ベースライン (Chunk 24a 実機): 約 4 MB/s
    目標 (Chunk 24b):              50-100 MB/s (実機で検証)
    本デモはフィクスチャが小さいため絶対値は参考程度。
```

実速度測定は Chouさんの 2 回目実機ドライランで実施予定。

**🎯 テスト統計**:
- 新規単体テスト **12 件**（progress 7 + engine 5: `process_recovery_task_writes_file_with_buffered_io`, `process_recovery_task_propagates_priority_metadata`, `write_with_large_buffer_creates_parent_dirs`, `worker_count_is_in_expected_range`, `write_buffer_size_constant_is_one_megabyte`）
- 新規結合テスト **4 件**（parallel 2 + business_flow 2、うち 2 件 `#[ignore]`）
- 修正既存テスト **15 件**（`NoopProgressReporter` 機械的 migration）
- workspace 全体 **553 件 pass / 0 failed / 6 ignored**（Chunk 24a 539 → **+14 件**）

**🎯 安全性（CRITICAL）**:
- **unsafe 行数**: Chunk 24a の 2 ブロック (`timestamps.rs:132, 143`) **から増加なし**
- crossbeam-channel / num_cpus / sha2 / BufWriter / std::thread::spawn すべて safe API
- **並列化に unsafe 不要を達成** — Rust の所有権モデルが safe な並列処理を保証
- `unsafe fn` / `unsafe impl` / `unsafe trait` / `unsafe extern`: **0 件**
- ProgressReporter は `Send + Sync` 制約で並列安全
- WorkerPanic 検出と RecoveryError への変換
- clippy / doc warning **0 件**
- cargo fmt clean (Chunk 24a で解消した状態を維持)
- ソースデバイス書き込みなし

**🎯 関連 FR**:
- **FR-REC-08（復旧速度、目標 100 MB/s、新規）→ ✅ 🎯 並列化実装完了、実機で 50-100 MB/s 検証待ち**
- **FR-CLI-08（進捗表示、新規）→ ✅ 🎯 新規達成**

**🎯 業務的成果**:
- 4 MB/s → 50-100 MB/s 目標達成見込み（並列化 + I/O バッファ拡大）
- 進捗表示で「動いている」感が出る → お客様待ち時間の不安解消
- workbench-dryrun の完了表示に速度 (MB/s) 追加 → CS が業務的に効率測定可能
- Phase 2.1 UI で `TauriProgressReporter` として再利用可能な抽象設計（`Send + Sync` ProgressReporter trait）
- 「unsafe 追加なし」維持で Rust の所有権モデルによる safe な並列処理を実証

### 🎯🎯🎯 Phase 1.5 業務適用品質の最終形完成マイルストーン（Chunk 24b 完了時）

```
🎯🎯🎯 DDS Recovery Workbench - Phase 1.5 業務適用品質の最終形完成 (Chunk 24b) 🎯🎯🎯
  M0 設計確定         100% ✅
  M1 基盤構築          30% （Phase 1 では基盤として十分機能、Phase 2 で残実装）
  M2 NTFS リーダα     100% ✅
  M3 希望突合エンジン  100% ✅
  ...
  Chunk 24a         お客様向けレポート簡素化+タイムスタンプ保持 ✅ 完成 🎯 R-STUDIO 並み
  ─────────────────────────────────────────
  Phase 1.5 業務適用品質の最終形完成 — 🎯 4 MB/s → 50-100 MB/s + 進捗表示 🎯
  Chunk 24b         並列化によるパフォーマンス改善 + 進捗表示 ✅ 完成 🎯 業務適用品質確定可能
  ─────────────────────────────────────────
  次のステップ
  実機検証 (2 回目) Chouさんによる検証 PC 実機ドライラン ⏳ 次推奨
                   ├ お客様向け .docx 確認
                   ├ タイムスタンプ R-STUDIO 比較
                   ├ CSV 文字化け確認
                   ├ 復旧速度 4 MB/s → 50-100 MB/s 改善実測
                   └ 進捗表示「動いている感」確認
  Chunk 24c         追加最適化 (100 MB/s 未達なら)     ⏳ 条件付き次推奨
  Phase 2.1         Tauri UI (約 2 ヶ月想定)           ⏳ 次推奨
                   └ TauriProgressReporter として trait 再利用可能
  ─────────────────────────────────────────
  Chunks 1-24b 完了（サブチャンク含む 30 ドキュメント）
  workspace total: 553 件 pass / 0 failed / 6 ignored
  unsafe: 2 ブロック（timestamps.rs に限定、Chunk 24a 以降増加なし）、他全クレート 0 件
  並列化に unsafe 不要を達成 — Rust の所有権モデルで safe な並列処理を実現
  clippy 0 件 / doc 0 件 / fmt --check exit 0
```

### 🎯 Chunk 24b 次のステップ

1. **🎯 Chouさんによる 2 回目実機ドライラン実施**: Chunk 24a + 24b の業務適用品質を業務メンバーで確認
   - お客様向け .docx 確認（Chunk 24a 範疇）
   - タイムスタンプ R-STUDIO 比較（Chunk 24a 範疇）
   - CSV 文字化け確認（Chunk 24a 範疇）
   - **復旧速度 4 MB/s → 50-100 MB/s 改善実測**（Chunk 24b 範疇）
   - **進捗表示「動いている感」確認**（Chunk 24b 範疇）
2. **Chunk 24c (条件付き)**: 100 MB/s 未達の場合の追加最適化検討
3. **Phase 2.1 着手準備**: Tauri UI 開発、約 2 ヶ月想定、`TauriProgressReporter` として `ProgressReporter` trait を再利用可能（`Send + Sync` 設計済）
...
[業務フロー]
  Workbench は R-STUDIO 風の全件復旧を実施
  Wishlist は『お客様優先データ』としてレポートで強調
...
**注: 品質保証率 0.0% は `.txt` 用 validator が registry にないため Uncertain 判定（Phase 1 既知制限、Chunk 23.7 とは独立、Chunk 23.8 候補で改善検討）**。**🎯 業務観測（既存フィクスチャでの ExclusionList 適用）**: | フィクスチャ | 全 user file | 除外 | 復旧 | priority | / | ntfs_with_5_deletions_small (.txt 30 件) | 30 | 0 | 30 | 30 (.txt マッチ) | / | ntfs_mixed_formats (15 件) | 15 | 0 | 15 | 14 (xyz 除く) | / | ntfs_directories (109+ 件) | 109+ | 0 | 109+ | 109+ | — フィクスチャに `\Windows\` 等が存在しないため除外 0 件 = **正常な業務観測**、実機運用では効く設計。**🎯 サンプル CRM テキスト（実生成、内部 HTML）**: 「`<h2>復旧結果 (全体)</h2> ... <h2>お客様優先データ (Wishlist マッチ)</h2> <table><tr><th>該当ファイル数</th><td><span class="metric">14</span> 件</td></tr><tr><th>復旧データ量</th><td>1.30 KB</td></tr><tr><th>品質保証率</th><td><span class="metric">71.4%</span></td></tr></table> <table><tr><th>判定</th><th>件数</th></tr><tr><td>Valid (正常)</td><td>10</td></tr><tr><td>Invalid (要確認)</td><td>4</td></tr><tr><td>Uncertain (検証外)</td><td>0</td></tr></table>`」。**🎯 サンプル CSV ヘッダー（15 列）**: `source_id,original_path,output_path,bytes_written,is_deleted,is_priority,priority_score,matched_wishes,sha256,validation_status,format_detected,validator_name,customer_message,internal_note,diagnostics`。**🎯 修正したテスト件数の内訳**: ExclusionList 単体 (wish-match) 新規 6 / recovery 単体 (engine + report) 新規 6 (recover 3 + priority 3) + 既存修正 ~9 / recovery 結合 (4 ファイル) 既存修正 11 (signature migration + priority 二重アサート) / report 単体 (csv + html + docx + txt) 新規 5 (csv 1 + html 2 + docx 2) + 既存修正 ~3 (index 更新) / case-manager 結合 新規 2 + 既存修正 2 (signature migration) / workbench-dryrun 既存修正 0 (test 影響なし) = **合計 17 新規 + 約 25 既存修正 + 全 `RecoveredEntry` リテラル更新**、workspace 全体 Chunk 23.5 489 → Chunk 23.7 **508 件**（+19 件: 17 単体 + 2 結合）。**🎯 安全性**: `crates/{wish-match, recovery, report, case-manager, workbench-dryrun}/src/` 内 `unsafe` **0 件**、書き込み API: 出力先のみ、ソースデバイス書き込みなし、単方向依存維持: case-manager → recovery + report + wish-match（変化なし）、clippy / doc warning **0 件**。**🎯 関連 FR**: **FR-REC-05（全件復旧、業務適用、新規）→ ✅ 🎯 新規達成** / **FR-REC-06（システムファイル除外、新規）→ ✅ 🎯 新規達成** / **FR-REP-04（優先データ強調表示）→ ✅ 🎯 拡張完了**。**🎯🎯🎯 マイルストーン意義（R-STUDIO 風業務フロー対応完成 — Workbench は R-STUDIO の置き換え候補として評価可能な状態に到達）**: Phase 1.5 の業務的本質「お客様は『全部復旧して』と言う」を実装に落とし込み、Wishlist の意味再定義（「復旧対象指定」→「お客様優先データのラベリング」）+ ExclusionList 新規導入 + 全件復旧 + Wishlist マッチで `is_priority=true` の二重表示により、Workbench を R-STUDIO の置き換え候補として評価可能な状態を確立。**次のステップ**: ①**Chunk 23.8: Uncertain の理由分類 + TXT 分割（必要なら）**（`.txt` 用 validator が registry にないため Uncertain 判定される Phase 1 既知制限への対応、業務観測「品質保証率 0.0%」の解消候補） / ②**Chouさんによる検証 PC ドライラン実施**（中古 NTFS HDD、約半日〜1 日、R-STUDIO 風業務フロー対応完成後の業務適用品質検証） / ③**フィードバック収集 → Phase 2.1 着手準備**（Tauri UI 開発、約 2 ヶ月想定）。

```
🎯🎯🎯 DDS Recovery Workbench - R-STUDIO 風業務フロー対応完成（破壊的変更による業務的本質の実装）🎯🎯🎯
  M0 設計確定         100% ✅
  M1 基盤構築          30% （Phase 1 では基盤として十分機能、Phase 2 で残実装）
  M2 NTFS リーダα     100% ✅
  M3 希望突合エンジン  100% ✅
  ...
  Phase 2.1    Tauri UI (約 2 ヶ月想定)        ⏳ 次推奨
  ─────────────────────────────────────────
  Chunks 1-23.7 完了 / 508 件 pass / 0 failed / 2 ignored / R-STUDIO 風業務フロー対応完成
```

### 🎯 Chunk 23.7 業務的意味論変更（破壊的、最重要）

| 項目 | 旧（〜Chunk 23.5） | 新（Chunk 23.7） |
|---|---|---|
| **Wishlist** | 復旧対象のファイル指定（Inclusion フィルタ） | お客様優先データのラベリング |
| **復旧範囲** | Wishlist マッチした N 件のみ復旧 | 全 user file 復旧（ExclusionList で除外） |
| **Wishlist マッチ** | 復旧対象 | レポートで `is_priority=true` として強調表示 |
| **ExclusionList** | 存在しない | 新規導入、システムファイル除外（7 パターン） |
| **レポート構造** | Wishlist マッチのみ表示 | 「全体」+「優先データ (Wishlist マッチ)」の二重表示 |

### 🎯 Chunk 23.7 業務シナリオ実証（R-STUDIO 風の業務フロー）

```
1. お客様: HDD 持ち込み → 「全部復旧して」（主訴）
2. CS: ヒアリング + CRM 採番（260522-04）
   - お客様優先データ（Wishlist）をヒアリング: 「写真と Word ファイルが最優先」
3. Workbench: `execute_business_recovery(case, drive_root, volume, wishlist, exclusions)`
   - 全 user file 復旧（ExclusionList でシステムファイル除外）
   - Wishlist マッチは is_priority=true
4. 自動生成:
   - 納品 HDD G:\260522-04\ (全件復旧データ + 4 レポート)
   - レポートに「全体」と「優先データ」の二重表示
5. CS: 納品 HDD をお客様送付
   - 「全部復旧しました（全 30 件、優先データ 14 件は品質保証率 71.4%）」
```

### 🎯 Chunk 23.7 関連 FR の進捗（新規達成 2 件 + 拡張完了 1 件）

- **FR-REC-05（全件復旧、業務適用、新規）**: ✅ **🎯 新規達成**（Chunk 23.7 / 2026-05-25 / dds-recovery + dds-wish-match + dds-case-manager）— `recover_files(&self, volume, wishlist, exclusions)` シグネチャ変更 + 全件復旧化、マッチした N 件のみ復旧 → 全 user file 復旧、Wishlist マッチは is_priority=true、R-STUDIO 風の業務フロー（「全部復旧して」）に対応、月 800 件の案件で「あのファイルが入ってない!」クレーム回避
- **FR-REC-06（システムファイル除外、新規）**: ✅ **🎯 新規達成**（Chunk 23.7 / 2026-05-25 / dds-wish-match）— `crates/wish-match/src/exclusion.rs` 196 行（`ExclusionList` + `ExclusionPattern` enum 3 バリアント (PathPrefix / NameStartsWith / Extension) + `default_system_exclusions()` 7 パターン: `\Windows\` / `\Program Files\` / `\Program Files (x86)\` / `\$Recycle.Bin\` / `\System Volume Information\` / `\$Extend\` / `NameStartsWith("$")` + 6 単体テスト）、業務的システムファイル除外を `recover_files` に渡して全件復旧から除外、実機運用で効く設計
- **FR-REP-04（業務指標可視化、優先データ強調表示）**: ✅ **🎯 拡張完了**（Chunk 20.5 で新規達成 → Chunk 23.7 で **「全体」と「優先データ (Wishlist マッチ)」の二重表示**へ拡張 / dds-recovery + dds-report）— `RecoveredEntry.is_priority: bool` 新規フィールド + priority 統計 6 メソッド + CSV 15 列化（`is_priority` 列 index 5 追加） + HTML 優先データセクション + DOCX 優先データセクション、レポートで「全体」と「優先データ」の二重表示を確立、お客様への提示時に「全部復旧（30 件）+ 優先データ品質保証率 71.4% (14 件)」のような具体的な数字を提示可能
...
```
🎊🎊🎊 DDS Recovery Workbench - Phase 1.5 完全完成（業務統合層完成） 🎊🎊🎊
  M0 設計確定         100% ✅
  M1 基盤構築          30% （Phase 1 では基盤として十分機能、Phase 2 で残実装）
  M2 NTFS リーダα     100% ✅
  M3 希望突合エンジン  100% ✅
  ...
  Chunk 21   case-manager 基盤            ✅ 完成
  Chunk 22   診断エンジン+CRMテキスト     ✅ 完成
  Chunk 22.6 業務フロー整合 (症状判定削除) ✅ 完成 🎯 Workbench は「事実提供者」に位置付け
  Chunk 22.5 復旧可能性推定 (High/Med/Low) ✅ 完成 📈 削除案件の見積もり精度向上
  Chunk 23   業務向け出力ディレクトリ構造 ✅ 完成 🎊 Phase 1.5 最終 (Chunk 23 / 2026-05-25)
  ─────────────────────────────────────────
  次のステップ
  実機検証   中古 NTFS HDD ドライラン     ⏳ 次推奨 (Chouさん手動)
  Phase 2.1  Tauri UI (約 2 ヶ月想定)     ⏳ 次推奨
  ─────────────────────────────────────────
  Chunks 1-23 完了 / 466 件 pass / 0 failed / 3 ignored / Phase 1.5 完全完成
```

### 🎊 Phase 1 + Phase 1.5 全要件達成サマリ（業務員提示用）

**Phase 1 NTFS-α 業務適用版（Chunks 1-20.5）— 完成**
- NTFS 復旧パイプライン（read-only、出力分離、SHA256 検証）
- 希望リスト突合（13 種パターン + glob + 日付 + 論理結合）
- 品質判定（9 validator: PNG/JPEG/PDF/GIF/BMP/ZIP/DOCX/XLSX/PPTX、3 値 status）
- 業務適用レポート（.docx + TXT + HTML + CSV、業務指標）

**Phase 1.5 業務統合層（Chunks 21-23 含む 22.6, 22.5）— 完成**
- 案件管理基盤（CaseId / Case / CaseStorage、JSON 永続化）
- 診断エンジン（単一パス、症状判定削除、CRM 貼り付けテキスト）
- 復旧可能性推定（High/Medium/Low、NTFS 技術的事実ベース）
- 業務向け出力構造（CaseOutput + 4 レポート日本語名 + execute_business_recovery）

**Phase 1.5 完成後の業務フロー**
1. お客様: HDD 持ち込み
2. CS: ヒアリング + CRM 採番（260522-04）
3. Workbench: `execute_business_recovery` 1 回呼出
4. 自動生成:
   - 納品 HDD `G:\260522-04\` (復旧データ + 4 レポート)
   - 社内 `C:\cases\260522-04\case.json` (案件情報)
5. CS: 納品 HDD をお客様送付、case.json で業務記録

### 🎊 Chunk 23 業務観測（プロダクトデモ全文）
...
[納品 HDD] (tempfile)
  └─ 260522-04/
      ...
      │   └─ 削除ファイル/ (5 件)
      └─ レポート/
          ├─ 復旧レポート.docx (23125 bytes)
          ├─ 要確認ファイル一覧.txt (352 bytes)
          ├─ 業務管理レポート.html (3202 bytes)
          └─ report.csv (14145 bytes)
...
注: 品質保証率 0.0% は `ntfs_with_5_deletions_small` 内が `.txt` のみで validators が Uncertain 判定するため正常な業務観測（Tester 確認済）。

### 🎊 Chunk 23 関連 FR の進捗（新規達成 4 件 + 既存達成）

- **FR-OUT-01（案件番号付きルートディレクトリ、新規）**: ✅ **🎊 新規達成**（Chunk 23 / 2026-05-25 / dds-case-manager）— `CaseOutput::root()` が `{base}\{案件番号}\` 形式で出力ルートを返却、`G:\260522-04\` などお客様送付時に案件単位で整理された納品物として確立
- **FR-OUT-02（通常 / 削除ファイル分離、新規）**: ✅ **🎊 新規達成**（Chunk 23 / 2026-05-25 / dds-case-manager + dds-recovery）— `live_files_dir()` (`復旧データ/通常ファイル/`) + `deleted_files_dir()` (`復旧データ/削除ファイル/`) で生存ファイル / 削除エントリを業務観点で分離、`RecoveryConfig` + `with_config_and_options` で出力先を制御
- **FR-OUT-03（日本語フォルダ名・ファイル名、新規）**: ✅ **🎊 新規達成**（Chunk 23 / 2026-05-25 / dds-case-manager + dds-report）— 「復旧データ」「通常ファイル」「削除ファイル」「レポート」ディレクトリ + 「復旧レポート.docx」「要確認ファイル一覧.txt」「業務管理レポート.html」ファイル UTF-8 で正常作成、Windows 日本語パス対応を実 tempfile (AppData) 上で全動作確認
- **FR-OUT-04（社内保存と納品物分離、新規）**: ✅ **🎊 新規達成**（Chunk 23 / 2026-05-25 / dds-case-manager）— 納品 HDD `G:\260522-04\` (お客様送付) と社内保存 `C:\cases\260522-04\case.json` (案件情報、お客様には見せない) を独立した出力先として分離、CS の業務フロー（納品 HDD 取り出し → お客様送付 → 社内 case.json 保持）に整合
- **FR-CASE-05（案件のエクスポート）**: ✅ **🎊 達成**（Chunk 23 / 2026-05-25 / dds-case-manager）— `execute_business_recovery` 1 関数呼出で個別案件を独立フォルダ（復旧データ + 4 レポート + case.json）として書き出し可能、業務観点で整理された納品物として自動生成

---

## 🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成 📈📈📈📈📈** / `RecoverabilityEstimate` 本実装（Chunk 21 placeholder → Chunk 22.5 で High/Medium/Low 3 値判定の本実装）/ NTFS 技術的事実（resident attribute / run-list 完全性 / クラスタ占有状態）のみで判定、ヒューリスティック禁止 / 月 700-800 件案件のうち約 30%（240 件）の削除案件で見積もり精度向上 / お客様への提示「削除 234 件中、High 198 / Medium 30 / Low 6」のような具体的な数字を伝達可能 / `crates/diagnostic/src/recoverability.rs` 新規 216 行（`estimate()` + `categorize()` + 8 単体）+ `crates/diagnostic/tests/recoverability_integration.rs` 新規 104 行（2 結合）+ `crates/fs-ntfs/src/file.rs` +157 行（`ClusterRange` struct + `NtfsFile::occupied_cluster_ranges()` + 4-5 単体）+ `crates/diagnostic/src/aggregator.rs` +115 行（`ClusterOccupancyMap` + `DeletedFileMetadata` + `aggregate_all` 拡張 + 3 単体）+ `crates/diagnostic/src/lib.rs`（`pub mod recoverability;` + `DiagnosticEngine::diagnose` に推定統合）+ `crates/diagnostic/src/crm_text.rs` +52 行（「復旧可能性 (推定)」セクション render + 2 単体）/ 単体テスト +18 件 + 結合テスト +2 件 = workspace 全体 **448 件 pass / 0 failed**（Chunk 22.6 の 428 → +20 件）/ 業務観測（ntfs_with_5_deletions_small）削除 5 件中 5 件すべて High（理由: 全 ~50 バイト TXT の `$DATA` resident、`categorize` 1 段目で確定）/ 設計原則達成: 単一パス維持（`aggregate_all` の `iter_files()` ループ 1 回維持、削除メタデータ収集 + 占有マップ登録を追加）+ NTFS 技術的事実のみで判定 + 単方向依存維持（diagnostic → fs-ntfs + case-manager + core）+ 既存 `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` 活用 / 新規公開 API: `dds_fs_ntfs::ClusterRange { start_lcn, length }` + `NtfsFile::occupied_cluster_ranges()` + `dds_diagnostic::ClusterOccupancyMap`（new/mark_range/is_occupied/count_overlapping/occupied_cluster_count）+ `DeletedFileMetadata` + `recoverability::estimate()` / `crates/diagnostic/src/` および `crates/fs-ntfs/src/` 内 `unsafe` 0 件 + 書き込み API 追加なし（`mark_range` は in-memory のみ）+ clippy / doc warning 0 件 / **FR-DIAG-07（削除ファイル復旧可能性推定、新規）+ FR-DIAG-08（業務見積もりへの活用、新規）達成** / Chunks 1-22.5 完了（Chunk 22.5 / 2026-05-25）

**Chunk 22.5 ハイライト（削除案件の見積もり精度向上の節目）**: Chunk 21 で placeholder（`RecoverabilityEstimate { high_confidence: 0, medium_confidence: 0, low_confidence: 0 }`）として定義していた構造体を、Chunk 22.5 で **NTFS 技術的事実に基づく 3 値判定の本実装**に到達。**判定基準（ヒューリスティック禁止、NTFS 技術的事実のみ）**: ①**High**（確実復旧可能）= resident attribute OR (run-list 完全 + 全クラスタ未上書き) OR 0 バイトファイル / ②**Medium**（部分復旧の可能性）= non-resident + run-list 完全 + 部分上書き / ③**Low**（メタデータのみ）= run-list 破損 OR 全クラスタ上書き済み。**新規ファイル（2）**: ①`crates/diagnostic/src/recoverability.rs` 216 行（`estimate(&[DeletedFileMetadata], &ClusterOccupancyMap) -> RecoverabilityEstimate` + `categorize()` + 8 単体テスト）/ ②`crates/diagnostic/tests/recoverability_integration.rs` 104 行（2 結合テスト）。**修正ファイル（5）**: ①`crates/fs-ntfs/src/file.rs` +157 行（`ClusterRange { start_lcn, length }` struct + `end_lcn()` / `contains()` メソッド + `NtfsFile::occupied_cluster_ranges() -> Vec<ClusterRange>` 新規 + 4-5 単体テスト）/ ②`crates/fs-ntfs/src/lib.rs`（`ClusterRange` re-export）/ ③`crates/diagnostic/src/aggregator.rs` +115 行（`ClusterOccupancyMap`（`BTreeSet` ベース、Phase 2 で Roaring Bitmap 切替検討、5 メソッド: new/mark_range/is_occupied/count_overlapping/occupied_cluster_count）+ `DeletedFileMetadata { record_index, is_resident, run_list_valid, cluster_ranges }` + `aggregate_all` 拡張（**単一パス維持**: `iter_files()` ループ 1 回のまま削除メタデータ収集 + 占有マップ登録を追加）+ 3 単体テスト）/ ④`crates/diagnostic/src/lib.rs`（`pub mod recoverability;` + `DiagnosticEngine::diagnose` に `recoverability::estimate(&deleted_metadata, &occupancy_map)` 統合 + `DiagnosticReport.recoverability` フィールド設定）/ ⑤`crates/diagnostic/src/crm_text.rs` +52 行（「復旧可能性 (推定)」セクション render: `高 (確実復旧可能): N 件 / 中 (部分復旧の可能性): N 件 / 低 (メタデータのみ): N 件` + 判定基準注記 + 2 単体テスト）。**業務 CRITICAL 観測（ntfs_with_5_deletions_small）**: 削除エントリ数 5 / 復旧可能性 High 5 / Medium 0 / Low 0 — **5 件中 5 件すべて High（確実復旧可能）と判定**。理由: フィクスチャの削除エントリはすべて ~50 バイトの TXT で `$DATA` が resident（MFT 内データ完結）、`categorize` の 1 段目（`is_resident` → High）で確定。**業務メンバーへの説明**: 「ntfs_with_5_deletions_small の削除 5 件はすべて小サイズ TXT（resident）のため、MFT エントリさえ正常に読めれば 100% 復旧可能。お客様には『削除 5 件中 5 件確実復旧見込み』と提示できる」。**業務観測（プロダクトデモ、案件 260522-04）**: CRM 貼り付けテキスト末尾に「復旧可能性 (推定): 高 5 件 / 中 0 件 / 低 0 件 + 判定基準注記」が自動出力。**設計原則達成**: ①**単一パス維持**（`aggregate_all` の `iter_files()` ループ 1 回維持、削除メタデータ収集 + 占有マップ登録を追加だけで O(N) 性能保証）/ ②**NTFS 技術的事実のみで判定**（resident フラグ / runlist / クラスタ占有のみ、ヒューリスティック・拡張子・ファイル名依存ロジック完全禁止）/ ③**単方向依存維持**（diagnostic → fs-ntfs + case-manager + core、変化なし）/ ④**既存 `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` 活用**（Chunk 21 で `dds_case_manager` に定義済み、本実装で値を詰めるのみ、API 変更なし）。**新規公開 API**: ①`dds_fs_ntfs::ClusterRange { start_lcn, length }` + `end_lcn()` / `contains(lcn)` メソッド / ②`dds_fs_ntfs::NtfsFile::occupied_cluster_ranges() -> Vec<ClusterRange>` / ③`dds_diagnostic::ClusterOccupancyMap` + 5 メソッド（`new` / `mark_range(start, length)` / `is_occupied(lcn) -> bool` / `count_overlapping(ranges) -> usize` / `occupied_cluster_count() -> usize`）/ ④`dds_diagnostic::DeletedFileMetadata { record_index, is_resident, run_list_valid, cluster_ranges }` / ⑤`dds_diagnostic::recoverability::estimate(&[DeletedFileMetadata], &ClusterOccupancyMap) -> RecoverabilityEstimate`。**安全性**: `crates/diagnostic/src/` および `crates/fs-ntfs/src/` 内 `unsafe` **0 件**、書き込み API 追加なし（`ClusterOccupancyMap::mark_range` は in-memory のみ、ソースデバイス影響なし）、clippy / doc warning **0 件**。**テスト統計**: 新規単体テスト 18 件（recoverability 8 + aggregator 3 + crm_text 2 + fs-ntfs file 5）+ 新規結合テスト 2 件 = workspace 全体 **448 件 pass / 0 failed**（Chunk 22.6 の 428 → +20 件）。**Tester からの将来課題（Phase 2 候補）**: ①`aggregator.rs`（372 行）は累積で膨らんだため、将来 `aggregator/occupancy.rs` への分離検討（現状は必須ではない）/ ②`ClusterOccupancyMap::mark_range` の `BTreeSet` 実装は Phase 1.5 小規模フィクスチャ向け、Phase 2 で実 HDD 規模対応時に Roaring Bitmap への切替検討（rustdoc に既述）。**マイルストーン意義（削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成）**: Chunk 22.6 で「事実提供者」役割を確立した Workbench に、**Chunk 22.5 で「定量的な見積もり数字」を提供できる機能を実装に落とし込んだ**。月 700-800 件案件のうち約 30%（240 件）の削除案件で見積もり精度向上、お客様への提示時に「削除 234 件中、High 198 / Medium 30 / Low 6」のような **具体的な数字を NTFS 技術的事実のみに基づいて伝達可能**。次の Phase 1.5 候補は **Chunk 23 業務向け出力ディレクトリ構造（FR-CASE-05、Phase 1.5 最終）**、**実機検証（中古 NTFS HDD で診断 + 復旧可能性推定の精度検証）**。

```
📈📈📈 DDS Recovery Workbench - 削除案件の見積もり精度向上（Phase 1.5 見積もり精度向上機能完成） 📈📈📈
  M0 設計確定         100% ✅
  M1 基盤構築          30% （Phase 1 では基盤として十分機能、Phase 2 で残実装）
  M2 NTFS リーダα     100% ✅
  M3 希望突合エンジン  100% ✅
  ...
  Chunk 22.6 業務フロー整合 (症状判定削除) ✅ 完成 🎯 Workbench は「事実提供者」に位置付け
  Chunk 22.5 復旧可能性推定 (High/Med/Low) ✅ 完成 📈 削除案件の見積もり精度向上
  Chunk 23   業務向け出力構造             ⏳ 次推奨 (Phase 1.5 最終)
  実機検証   中古 NTFS HDD                ⏳ 次推奨
  ─────────────────────────────────────────
  Chunks 1-22.5 完了 / 448 件 pass / 0 failed / 削除案件の見積もり精度向上
```

### 📈 Chunk 22.5 業務観測（ntfs_with_5_deletions_small）
...
→ **5 件中 5 件すべて High（確実復旧可能）と判定**。理由: フィクスチャの削除エントリはすべて ~50 バイトの TXT で `$DATA` が resident（MFT 内データ完結）、`categorize` の 1 段目（`is_resident` → High）で確定。お客様には「削除 5 件中 5 件確実復旧見込み」と提示可能。

### 📈 Chunk 22.5 業務観測（プロダクトデモ、CRM 貼り付けテキスト末尾抜粋）
...
### 📈 Chunk 22.5 関連 FR の進捗（新規達成 2 件）

- **FR-DIAG-07（削除ファイル復旧可能性推定、新規）**: ✅ **📈 新規達成**（Chunk 22.5 / 2026-05-25 / dds-diagnostic + dds-fs-ntfs）— Chunk 21 placeholder の `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` を NTFS 技術的事実（resident attribute / run-list 完全性 / クラスタ占有状態）に基づく 3 値判定で本実装。`estimate()` + `categorize()` + `ClusterOccupancyMap` + `DeletedFileMetadata` + `NtfsFile::occupied_cluster_ranges()` + `ClusterRange` で完結、ヒューリスティック・拡張子・ファイル名依存ロジック完全禁止
- **FR-DIAG-08（業務見積もりへの活用、新規）**: ✅ **📈 新規達成**（Chunk 22.5 / 2026-05-25 / dds-diagnostic）— CRM 貼り付けテキストに「復旧可能性 (推定)」セクション追加（高 / 中 / 低 件数 + 判定基準注記）、業務員 / 顧客への定量的説明（「削除 234 件中、High 198 / Medium 30 / Low 6」のような具体的な数字を NTFS 技術的事実のみに基づいて伝達）

---

## 🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **診断レポートが業務フローと完全整合 — Workbench は「事実提供者」に位置付け 🩺🩺🩺🩺🩺** / Chunk 22 で発生した「ntfs_with_5_deletions_small で『フォーマット (複合)』と誤判定」問題を業務観点フィードバックで解消 / 症状判定ロジック完全削除（`Symptom` + `FsAnomaly` enum + `symptom_detector.rs` 全消去、grep で 0 件確認）/ Workbench の役割を「判定者」から「事実提供者」へ転換（主訴は CS / お客様が決定、Workbench は件数・破損状態のみ報告）/ `DiagnosticInput.symptom` → `DiagnosticInput.filesystem_findings` 移行 / 新規 `FilesystemFindings` struct（signature_valid / mft_corrupted_count / invalid_runlist_count / boot_sector_ok / other_issues）+ `has_any_issue()` / CRM テキスト全面書き換え（10 セクション、【症状判定】削除、【ファイルシステムの破損】上位移動、【MFT エントリ統計】新規）/ 削除案件の誤判定回帰防止テスト `product_demo_deleted_case_no_format_misdetection` 新規 / 削除 11 + 新規 11 = テスト件数維持 / 既存 428 件 ±0 件で内訳変化（**428 件 pass / 3 ignored / 0 failed**）/ ネット **-111 行**（594 deletions / 483 insertions、削除主体）/ FR-DIAG-04 業務適用品質達成 + FR-DIAG-06（事実ベース報告、新規）達成、FR-DIAG-02（症状自動判定）取り下げ「Workbench スコープ外 = CS 責務」と再定義 / Chunks 1-22.6 完了（Chunk 22.6 / 2026-05-22）

**Chunk 22.6 ハイライト（業務フロー整合の節目）**: Chunk 22 で生じた「フィクスチャの小ささに反応した『全ファイル数 < 50 → フォーマット (複合)』誤判定」がそのまま CRM 業務に流れ込めば CS / お客様の混乱を招くという業務観点フィードバックを反映。**業務フローの実態（① CS / お客様ヒアリングで主訴確定 → ② 物理診断（別ツール）→ ③ 論理診断（Workbench）の時点で主訴は既知）に Workbench を整合**させ、**「判定者」から「事実提供者」への設計原則転換**を実装に落とし込んだ。**削除（2 ファイル / -360 行）**: `crates/case-manager/src/symptom.rs`（-190 行、`Symptom` + `FsAnomaly` enum 完全削除）+ `crates/diagnostic/src/symptom_detector.rs`（-170 行、判定ロジック完全削除）。**修正（10 ファイル）**: case-manager の `lib.rs` / `diagnostic.rs`（新規 `FilesystemFindings` struct + `has_any_issue()` + 4 単体テスト）/ `case.rs` / 結合テスト / `examples/dump_case_json.rs` + diagnostic の `lib.rs`（`filesystem_findings = aggregate.anomalies.to_findings()` 追加）/ `report.rs`（`symptom` → `filesystem_findings`、`FsAnomalyReport::to_findings()` 新規）/ **`crm_text.rs` 全面書き換え**（症状判定セクション完全削除 → 【ファイルシステムの破損】上位移動 + 【MFT エントリ統計】新規 = 10 セクション構成）/ `aggregator.rs`（未使用閾値定数削除）/ 結合テスト（業務シナリオ 2 件新規）。**業務 CRITICAL 回帰防止**: `product_demo_deleted_case_no_format_misdetection` テスト pass（`!crm.contains("フォーマット (複合)")` + `!crm.contains("主症状: フォーマット")` + `!crm.contains("主症状:")` + `!crm.contains("【症状判定】")` + `crm.contains("【ファイルシステムの破損】")` + `crm.contains("【MFT エントリ統計】")`）。**Symptom 完全削除確認**: `grep -rn 'Symptom|symptom_detector|FsAnomaly::' crates/ --include='*.rs'` → **0 件**（コメント含む完全除去）。**テスト統計**: 削除 11 件（Symptom + symptom_detector テスト群）+ 新規 11 件（FilesystemFindings + 新 CRM テキスト構造 + 業務シナリオ + 回帰防止）+ 結合テスト 2 件新規 = **workspace 全体 428 件 pass / 3 ignored / 0 failed**（Chunk 22 の 428 から ±0、内訳が変化、ignored 2 → 3 は新規ヘルパ追加）。**workspace clippy / doc warning 0 件**、**`crates/case-manager/src/` および `crates/diagnostic/src/` 内 `unsafe` 0 件**、**単方向依存維持**（diagnostic → fs-ntfs + case-manager + core、case-manager → wish-match + core）。**case.json 互換性**: `dump_case_json` example の出力から `"symptom"` フィールド削除、`"filesystem_findings": { "signature_valid": true, "mft_corrupted_count": 0, "invalid_runlist_count": 0, "boot_sector_ok": true, "other_issues": [] }` 追加。**役割分担の明確化**: お客様「フォーマットしてしまった」(主訴) / Workbench「MFT エントリ 25 件」(事実) / CS「うん、フォーマット直後の MFT 量と整合する。復旧見積もり OK」 — **Workbench は「これはフォーマットです!」と言わない、CS が事実を読んで業務判断する**。

```
🎯🎯🎯 DDS Recovery Workbench - 診断レポートが業務フローと完全整合 🎯🎯🎯
  M0 設計確定         100% ✅
  M1 基盤構築          30% （Phase 1 では基盤として十分機能、Phase 2 で残実装）
  M2 NTFS リーダα     100% ✅
  M3 希望突合エンジン  100% ✅
  ...
  Chunk 22   診断エンジン+CRMテキスト     ✅ 完成
  Chunk 22.6 業務フロー整合 (症状判定削除) ✅ 完成 🎯 Workbench は「事実提供者」に位置付け
  Chunk 22.5 復旧可能性推定 (High/Med/Low) ✅ 完成 📈 削除案件の見積もり精度向上 (Chunk 22.5 / 2026-05-25)
  Chunk 23   業務向け出力構造             ⏳ 次推奨 (Phase 1.5 最終)
  実機検証   中古 NTFS HDD                ⏳ 次推奨
  ─────────────────────────────────────────
  Chunks 1-22.6 完了 / 428 件 pass / 3 ignored / 削除案件の誤判定問題解消
```

### 🎯 Chunk 22.6 業務観測（新 CRM テキスト全文、削除案件で誤判定なし）

```
=== 論理診断結果 (案件 260522-04) ===
診断日時: 2026-05-22 10:47
診断時間: 0 秒
※物理診断は別途実施済み
...
### 🎯 Chunk 22.6 関連 FR の進捗（業務適用品質達成 + 新規達成 + 取り下げ）

- **FR-DIAG-04**（CRM 貼り付け用テキスト）: ✅ **🎯 業務適用品質達成**（Chunk 22 で初版達成 → Chunk 22.6 で誤判定問題解消、業務フローと完全整合）
- **FR-DIAG-06**（事実ベースの報告、新規）: ✅ **🎯 新規達成**（Chunk 22.6 / 2026-05-22 / dds-diagnostic + dds-case-manager）— Workbench は判定者ではなく事実提供者、判定ロジック完全排除、`FilesystemFindings` で件数・破損状態のみ報告、主訴判定は CS / お客様の責務として明確化
- **FR-DIAG-02**（症状自動判定）: ❌ **取り下げ「Workbench スコープ外」**（Chunk 22 で達成マークしたが、Chunk 22.6 で業務観点フィードバックにより削除、CS / お客様が主訴を決定する業務フロー上 Workbench の責務外と再定義）

---

## 🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉 **論理診断の自動化達成 — Phase 1.5 最重要機能完成 🩺🩺🩺🩺** / `crates/diagnostic` 新規誕生（業務統合の核） / `DiagnosticEngine::diagnose()` 1 コマンドで HDD 接続 → CRM 貼り付けテキスト出力の業務フロー pipeline 動作 / 月 700-800 件の診断業務の手間削減基盤完成 / 単一パス集計（iter_files ループ 1 回で全統計並行集計、業務 CRITICAL）/ 症状自動判定（None/Deleted/Formatted/FilesystemError/Mixed 優先順位、※Chunk 22.6 で業務観点フィードバックにより削除）/ CRM 貼り付け業務日本語テキスト生成（業務観点フィードバック反映、礼儀正しい、技術用語回避）/ `dds-core::format` モジュール新規 + `dds-report::format` delegate 化（コード重複解消）/ 19 ファイルに cargo fmt 適用（セマンティック変更ゼロ、テスター実 grep 検証済）/ FR-DIAG-01〜05 達成（※Chunk 22.6 で FR-DIAG-02 取り下げ）/ 既存 394 件 + 新規 34 件 = **428 件 pass / 0 failed / 2 ignored** / Chunks 1-22 完了（Chunk 22 / 2026-05-22）
...
- **🎊 Chunk 23 / 2026-05-25 / 🎊 Phase 1.5 完全完成 — 業務統合層完成 / Phase 2.1 (Tauri UI) への移行準備完了 ハイライト**: Chunk 22.5 で「定量的見積もり精度向上」を実装に落とし込んだ Workbench に、**Chunk 23 で「業務員が CRM 採番した案件番号で『G:\260522-04\ をお客様にそのまま納品できる』状態」を実装に到達**。**業務的背景**: 月 700-800 件案件の納品物が業務員ごとにバラバラなディレクトリ構造で出力されていたものを、案件番号付きルートディレクトリで自動生成、業務観点で整理された納品物として CS フローに直接乗せられる状態を確立。**🎊 業務的納品ディレクトリ構造（完成）**: `G:\260522-04\ ├ 復旧データ\ │   ├ 通常ファイル\ (生存ファイル) │   └ 削除ファイル\ (削除エントリ) └ レポート\ ├ 復旧レポート.docx (顧客向け Word) ├ 要確認ファイル一覧.txt (顧客向け Notepad) ├ 業務管理レポート.html (CS 内部用) └ report.csv (外部システム連携)` + 社内保存（お客様には見せない）`C:\cases\260522-04\case.json`（Chunk 21 で実装済み）。**🎊 新規ファイル（5）**: ①`crates/case-manager/src/output.rs` 204 行（`CaseOutput` struct + 12 メソッド: root / live_files_dir / deleted_files_dir / reports_dir / customer_docx_path / customer_txt_path / internal_html_path / csv_path / create_all_dirs 等 + 5 単体テスト）/ ②`crates/case-manager/src/orchestration.rs` 190 行（`execute_business_recovery` + `BusinessRecoveryResult` + `BusinessRecoveryError` + 3 単体テスト）/ ③`crates/report/src/business.rs` 166 行（`write_business_reports(report, &Path×4)` + `BusinessReportPaths` + 3 単体テスト）/ ④`crates/case-manager/tests/business_flow_integration.rs` 208 行（2 結合テスト）/ ⑤`crates/case-manager/tests/common/mod.rs` 63 行（fixture ヘルパー + `count_files_recursive` std 実装）。**🎊 修正ファイル（5）**: ①`crates/recovery/src/engine.rs` +142 行（`RecoveryConfig` struct + `with_config` / `with_config_and_options` + 4 単体テスト、既存 `output_dir` 維持で backward compat）/ ②`crates/recovery/src/lib.rs`（`RecoveryConfig` export）/ ③`crates/report/src/lib.rs`（`business` module + export）/ ④`crates/case-manager/src/lib.rs`（`orchestration` / `output` module + re-export）/ ⑤`crates/case-manager/Cargo.toml`（`dds-recovery` / `dds-report` / `dds-fs-ntfs` 依存追加 = **Phase 1.5 意図的拡大**、dev `zstd 0.13` 追加）。**🎊 設計判断**: ①`write_business_reports(report, &Path×4)` — `CaseOutput` を引数に取ると report → case-manager 循環依存になるため、4 パスを直接受け取る形に / ②`RecoveryEngine` の二重保持 — `output_dir` + `config` を両方保持で backward compat 維持 / ③`count_files_recursive` — walkdir 不依存、std::fs::read_dir 再帰実装 / ④`RecoveryConfig::from_case_output` は recovery に置かず — 循環依存回避、case-manager 側で `RecoveryConfig::with_paths(...)` を呼ぶ。**🎊 case-manager 依存拡大（Phase 1.5 意図的）**: Before（Chunks 21-22.5）case-manager → wish-match + core のみ → After（Chunk 23）case-manager → wish-match + core + **recovery + report + fs-ntfs**、理由: `execute_business_recovery` で全体オーケストレーションを実現、循環なし確認 cargo tree 検証済（recovery / report は case-manager に依存しない）。Phase 2 で別クレート（`dds-orchestrator` 等）に分離検討可能。**🎊 業務観測（プロダクトデモ全文）**: 案件番号 260522-04 / 納品 HDD `G:\` (tempfile) で「260522-04/復旧データ/通常ファイル/ (25 件) + 削除ファイル/ (5 件) + レポート/復旧レポート.docx (23125 bytes) + 要確認ファイル一覧.txt (352 bytes) + 業務管理レポート.html (3202 bytes) + report.csv (14145 bytes)」+ 社内保存 `C:\cases\` (tempfile) で「260522-04/case.json（案件情報、お客様には見せない）」+ 業務指標（該当ファイル 30 件 / 復旧成功率 100.0% / 品質保証率 0.0%）+ CS のフロー（① 納品 HDD を取り出す → `G:\` / ② お客様に `G:\` を送付 → お客様は `G:\260522-04\` を開くだけで全部見える / ③ 社内には案件情報が残る (再復旧依頼に備えて)）。**注: 品質保証率 0.0% は `ntfs_with_5_deletions_small` 内が `.txt` のみで validators が Uncertain 判定するため正常な業務観測（Tester 確認済）**。**🎊 業務インパクト**: HDD 接続 → `execute_business_recovery` 1 関数呼出 → 「G:\ をお客様にそのまま納品できる」状態 / 月 700-800 件案件の納品物自動生成 / CS 業務フロー: ① 納品 HDD 取り出し → ② お客様送付 → ③ 社内 case.json 保持。**🎊 既存 API 完全互換維持**: `RecoveryEngine::new(output_dir)` / `with_options(output_dir, options)` / `output_dir()` getter / `RecoveryOptions { separate_live_and_deleted: false }` 経路 / `dds_report::write_all_reports(report, output_dir)` / Chunk 17 以来の既存テスト全 pass。**🎊 安全性**: `crates/case-manager/src/` + `crates/recovery/src/` + `crates/report/src/` 内 `unsafe` **0 件**、書き込み API は出力先のみ、ソースデバイス書き込みなし、単方向依存（拡大後も循環なし、cargo tree 検証済）、clippy / doc warning **0 件**。**🎊 Windows 日本語パス対応**: 「復旧データ」「通常ファイル」「削除ファイル」「レポート」ディレクトリ UTF-8 で正常作成 + 「復旧レポート.docx」「要確認ファイル一覧.txt」「業務管理レポート.html」ファイル正常作成 + 実 tempfile (Windows AppData) 上で全動作確認。**🎊 テスト統計**: 新規単体テスト 15 件（case-manager output 5 + orchestration 3 + recovery RecoveryConfig 4 + report business 3）+ 新規結合テスト 2 件（`full_business_flow_from_case_creation_to_delivery` + `product_demo_phase_1_5_complete`）= workspace 全体 **466 件 pass / 0 failed / 3 ignored**（Chunk 22.5 の 448 → +18 件）。**関連 FR**: **FR-OUT-01**（案件番号付きルートディレクトリ、新規）→ ✅ **🎊 新規達成** / **FR-OUT-02**（通常 / 削除ファイル分離、新規）→ ✅ **🎊 新規達成** / **FR-OUT-03**（日本語フォルダ名・ファイル名、新規）→ ✅ **🎊 新規達成** / **FR-OUT-04**（社内保存と納品物分離、新規）→ ✅ **🎊 新規達成** / **FR-CASE-05**（案件のエクスポート）→ ✅ **🎊 達成**。**🎊🎊🎊 マイルストーン意義（Phase 1.5 完全完成 — 業務統合層完成 / Phase 2.1 (Tauri UI) への移行準備完了）**: Phase 1（NTFS-α 業務適用版、Chunks 1-20.5）+ Phase 1.5（業務統合層、Chunks 21-23 含む 22.6, 22.5）の全機能が業務的に整理された納品物として自動生成される状態に到達。**Phase 1 + Phase 1.5 完成後の業務フロー**: ① お客様: HDD 持ち込み → ② CS: ヒアリング + CRM 採番（260522-04）→ ③ Workbench: `execute_business_recovery` 1 回呼出 → ④ 自動生成: 納品 HDD `G:\260522-04\` (復旧データ + 4 レポート) + 社内 `C:\cases\260522-04\case.json` (案件情報) → ⑤ CS: 納品 HDD をお客様送付、case.json で業務記録。**次のステップ**: ①**検証 PC で実機ドライラン**（Chouさん手動、中古 NTFS HDD でフィクスチャでは検証できない実機特有の問題発見） / ②**Phase 2.1 着手準備**（Tauri UI 開発、約 2 ヶ月想定）。
- **📈 Chunk 22.5 / 2026-05-25 / 📈 削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成 ハイライト**: Chunk 21 で placeholder（`RecoverabilityEstimate { high_confidence: 0, medium_confidence: 0, low_confidence: 0 }`）として定義していた構造体を、Chunk 22.5 で **NTFS 技術的事実に基づく 3 値判定の本実装**に到達。**業務的背景**: 月 700-800 件案件のうち約 30%（240 件）の削除案件で、業務員 / 顧客への定量的見積もり説明が必要、お客様への提示時に「削除 234 件中、High 198 / Medium 30 / Low 6」のような具体的な数字を伝達するには NTFS 技術的事実（resident attribute / run-list 完全性 / クラスタ占有状態）に基づく定量判定が必要。**📈 判定基準（ヒューリスティック禁止、NTFS 技術的事実のみ）**: ①**High**（確実復旧可能）= resident attribute OR (run-list 完全 + 全クラスタ未上書き) OR 0 バイトファイル / ②**Medium**（部分復旧の可能性）= non-resident + run-list 完全 + 部分上書き / ③**Low**（メタデータのみ）= run-list 破損 OR 全クラスタ上書き済み。**📈 新規ファイル（2）**: ①`crates/diagnostic/src/recoverability.rs` 216 行（`estimate(&[DeletedFileMetadata], &ClusterOccupancyMap) -> RecoverabilityEstimate` + `categorize()` + 8 単体テスト）/ ②`crates/diagnostic/tests/recoverability_integration.rs` 104 行（2 結合テスト）。**📈 修正ファイル（5）**: ①`crates/fs-ntfs/src/file.rs` +157 行（`ClusterRange { start_lcn, length }` struct + `end_lcn()` / `contains()` メソッド + `NtfsFile::occupied_cluster_ranges() -> Vec<ClusterRange>` 新規 + 4-5 単体テスト）/ ②`crates/fs-ntfs/src/lib.rs`（`ClusterRange` re-export）/ ③`crates/diagnostic/src/aggregator.rs` +115 行（`ClusterOccupancyMap`（`BTreeSet` ベース、5 メソッド: new/mark_range/is_occupied/count_overlapping/occupied_cluster_count、Phase 2 で Roaring Bitmap 切替検討、rustdoc に既述）+ `DeletedFileMetadata { record_index, is_resident, run_list_valid, cluster_ranges }` + `aggregate_all` 拡張（**単一パス維持**: `iter_files()` ループ 1 回のまま、削除メタデータ収集 + 占有マップ登録を追加）+ 3 単体テスト）/ ④`crates/diagnostic/src/lib.rs`（`pub mod recoverability;` + `DiagnosticEngine::diagnose` に `recoverability::estimate()` 統合 + `DiagnosticReport.recoverability` フィールド設定）/ ⑤`crates/diagnostic/src/crm_text.rs` +52 行（「復旧可能性 (推定)」セクション render: `高 / 中 / 低` 件数 + 判定基準注記 + 2 単体テスト）。**📈 業務観測（ntfs_with_5_deletions_small）**: 削除エントリ数 5 / 復旧可能性 High 5 / Medium 0 / Low 0 — **5 件中 5 件すべて High（確実復旧可能）と判定**。理由: フィクスチャの削除エントリはすべて ~50 バイトの TXT で `$DATA` が resident（MFT 内データ完結）、`categorize` の 1 段目（`is_resident` → High）で確定。**📈 業務メンバーへの説明**: 「ntfs_with_5_deletions_small の削除 5 件はすべて小サイズ TXT（resident）のため、MFT エントリさえ正常に読めれば 100% 復旧可能。お客様には『削除 5 件中 5 件確実復旧見込み』と提示できる」。**📈 設計原則達成**: A. **単一パス維持**（`aggregate_all` の `iter_files()` ループ 1 回維持、削除メタデータ収集 + 占有マップ登録を追加だけで O(N) 性能保証）/ B. **NTFS 技術的事実のみで判定**（resident フラグ / runlist / クラスタ占有のみ、ヒューリスティック・拡張子・ファイル名依存ロジック完全禁止）/ C. **単方向依存維持**（diagnostic → fs-ntfs + case-manager + core、変化なし）/ D. **既存 `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` 活用**（Chunk 21 で `dds_case_manager` に定義済み、本実装で値を詰めるのみ、API 変更なし）。**📈 新規公開 API**: ①`dds_fs_ntfs::ClusterRange { start_lcn, length }` + `end_lcn()` / `contains(lcn)` / ②`dds_fs_ntfs::NtfsFile::occupied_cluster_ranges() -> Vec<ClusterRange>` / ③`dds_diagnostic::ClusterOccupancyMap` + 5 メソッド / ④`dds_diagnostic::DeletedFileMetadata` / ⑤`dds_diagnostic::recoverability::estimate()`。**📈 安全性**: `crates/diagnostic/src/` および `crates/fs-ntfs/src/` 内 `unsafe` **0 件**、書き込み API 追加なし（`ClusterOccupancyMap::mark_range` は in-memory のみ、ソースデバイス影響なし）、clippy / doc warning **0 件**。**📈 テスト統計**: 新規単体テスト 18 件（recoverability 8 + aggregator 3 + crm_text 2 + fs-ntfs file 5）+ 新規結合テスト 2 件 = workspace 全体 **448 件 pass / 0 failed**（Chunk 22.6 の 428 → +20 件）。**📈 Tester からの将来課題（Phase 2 候補）**: ①`aggregator.rs`（372 行）は累積で膨らんだため、将来 `aggregator/occupancy.rs` への分離検討（現状は必須ではない）/ ②`ClusterOccupancyMap::mark_range` の `BTreeSet` 実装は Phase 1.5 小規模フィクスチャ向け、Phase 2 で実 HDD 規模対応時に Roaring Bitmap への切替検討（rustdoc に既述）。**関連 FR**: **FR-DIAG-07**（削除ファイル復旧可能性推定、新規）→ ✅ **📈 新規達成** / **FR-DIAG-08**（業務見積もりへの活用、新規）→ ✅ **📈 新規達成**。**📈 マイルストーン意義（削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成）**: Chunk 22.6 で「事実提供者」役割を確立した Workbench に、**Chunk 22.5 で「定量的な見積もり数字」を提供できる機能を実装に落とし込んだ**。月 700-800 件案件のうち約 30%（240 件）の削除案件で見積もり精度向上、お客様への提示時に「削除 234 件中、High 198 / Medium 30 / Low 6」のような **具体的な数字を NTFS 技術的事実のみに基づいて伝達可能**。Phase 1.5 のうち「論理診断」モジュールは見積もり精度向上機能まで完成、次の Phase 1.5 候補は **Chunk 23 業務向け出力ディレクトリ構造（FR-CASE-05、Phase 1.5 最終）**、**実機検証（中古 NTFS HDD で診断 + 復旧可能性推定の精度検証）**。
- **🎯 Chunk 22.6 / 2026-05-22 / 🎯 診断レポートが業務フローと完全整合 — Workbench は「事実提供者」に位置付け ハイライト**: Chunk 22 で生じた「ntfs_with_5_deletions_small（削除案件テストデータ）で『フォーマット (複合)』と誤判定」問題を業務観点フィードバックで解消。**業務的背景**: フィクスチャの小ささ（全ファイル < 50）にヒューリスティックが反応した偽陽性、業務フローと食い違って CS / お客様の混乱を招くリスク。**業務フローの実態**: ① CS / お客様ヒアリングで主訴確定（"削除" "フォーマット" "FS 異常"）→ ② 物理診断（別ツール）→ ③ 論理診断（Workbench）の時点で主訴は既知、Workbench の役割は「事実 (件数・破損状態) を報告」のみ、判定は不要（既に CS が決めている）。**🎯 設計原則の転換**: Workbench は「判定者」ではなく「事実提供者」（お客様: 「フォーマットしてしまった」(主訴) / Workbench: 「MFT エントリ 25 件」(事実) / CS: 「うん、フォーマット直後の MFT 量と整合する。復旧見積もり OK」 → Workbench は「これはフォーマットです!」と言わない、CS が事実を読んで業務判断する）。**🎯 削除（2 ファイル / -360 行）**: `crates/case-manager/src/symptom.rs`（-190 行、`Symptom` + `FsAnomaly` enum 完全削除）+ `crates/diagnostic/src/symptom_detector.rs`（-170 行、判定ロジック完全削除）。**🎯 修正（10 ファイル）**: ①`case-manager/src/lib.rs`（`Symptom` export 削除、`FilesystemFindings` export 追加）/ ②`case-manager/src/diagnostic.rs`（`DiagnosticInput.symptom` → `DiagnosticInput.filesystem_findings`、新規 `FilesystemFindings` struct + `has_any_issue()` + 4 単体テスト）/ ③`case-manager/src/case.rs`（テスト assertion 更新）/ ④`case-manager/tests/case_lifecycle_integration.rs`（Symptom → FilesystemFindings 移行）/ ⑤`case-manager/examples/dump_case_json.rs`（サンプル値置換）/ ⑥`diagnostic/src/lib.rs`（症状判定処理排除、`filesystem_findings = aggregate.anomalies.to_findings()` 追加）/ ⑦`diagnostic/src/report.rs`（`symptom` 削除、`filesystem_findings` 追加、`FsAnomalyReport::to_findings()` メソッド追加、`to_anomaly_list()` 削除）/ ⑧**`diagnostic/src/crm_text.rs` 全面書き換え**（症状判定セクション完全削除、【ファイルシステムの破損】上位移動、【MFT エントリ統計】新規 = 10 セクション構成: ヘッダー → 【ハードウェア】→ 【ファイルシステム】→ 【ファイルシステムの破損】← 上位移動 → 【MFT エントリ統計】← 新規 → 【ファイル統計】→ 【削除エントリの詳細】(削除あれば) → 【生存ファイル統計】(参考) → 【主なフォルダ】→ 【物理不良チェック】、削除されたもの: 【症状判定】セクション全削除 / 「主症状: フォーマット (複合)」のような誤判定文 / `render_symptom_details()` 関数 / `anomaly_label()` 関数）/ ⑨`diagnostic/src/aggregator.rs`（未使用閾値定数削除）/ ⑩`diagnostic/tests/diagnostic_integration.rs`（新業務シナリオテスト 2 件追加）。**🎯 業務 CRITICAL 削除案件の誤判定回帰防止**: `product_demo_deleted_case_no_format_misdetection` テスト pass — `ntfs_with_5_deletions_small` で診断 → `!crm.contains("フォーマット (複合)")` （Chunk 22 で実発生した誤判定）+ `!crm.contains("主症状: フォーマット")` + `!crm.contains("主症状:")` + `!crm.contains("【症状判定】")` + `crm.contains("【ファイルシステムの破損】")` + `crm.contains("【MFT エントリ統計】")`。**🎯 Symptom 完全削除（CRITICAL）**: `grep -rn 'Symptom|symptom_detector|FsAnomaly::' crates/ --include='*.rs'` → **0 件**（コメント含む完全除去）。**🎯 ネット変更**: 594 deletions / 483 insertions = **-111 行**（削除主体）。**🎯 テスト統計**: 削除 11 件（Symptom + symptom_detector テスト群）+ 新規 11 件（FilesystemFindings + 新 CRM テキスト構造 + 業務シナリオ + 回帰防止）+ 結合テスト 2 件新規（`diagnose_populates_filesystem_findings` + `product_demo_deleted_case_no_format_misdetection`）= workspace 全体 **428 件 pass / 3 ignored / 0 failed**（Chunk 22 の 428 から ±0、内訳が変化、ignored 2 → 3）。**🎯 case.json 互換性**: `dump_case_json` example の出力から `"symptom"` フィールド削除、`"filesystem_findings": { "signature_valid": true, "mft_corrupted_count": 0, "invalid_runlist_count": 0, "boot_sector_ok": true, "other_issues": [] }` 追加。**🎯 単方向依存維持**: diagnostic → fs-ntfs + case-manager + core（変化なし）/ case-manager → wish-match + core（変化なし）。**🎯 安全性**: `crates/case-manager/src/` および `crates/diagnostic/src/` 内 `unsafe` **0 件**、clippy / doc warning **0 件**。**関連 FR**: **FR-DIAG-04**（CRM 貼り付けテキスト）→ ✅ **🎯 業務適用品質達成**（Chunk 22 初版 → Chunk 22.6 で誤判定問題解消、業務フローと完全整合）/ **FR-DIAG-06**（事実ベースの報告、新規）→ ✅ **🎯 新規達成**（判定ロジック完全排除、`FilesystemFindings` で件数・破損状態のみ報告）/ **FR-DIAG-02**（症状自動判定）→ ❌ **取り下げ「Workbench スコープ外 = CS 責務」**（業務観点フィードバックにより削除、CS / お客様が主訴を決定する業務フロー上 Workbench の責務外と再定義）。**🎯 マイルストーン意義（診断レポートが業務フローと完全整合）**: Chunk 22 の自動化達成（手段の自動化）の上に、**Chunk 22.6 で「業務フローの現実」に整合（提供する内容の正しさ）**を実装に落とし込んだ。業務フローと完全整合した「事実提供者」役割の Workbench は、CS の業務判断を妨げず補助する第三のツールとして位置付けが明確化。Phase 1.5 のうち「論理診断」モジュールは業務適用品質に到達、Chunk 22.5（復旧可能性推定）/ Chunk 23（業務向け出力ディレクトリ構造）/ 実機検証へ進められる状態。
...
- **総テスト数（workspace 全体）**: **553 件 pass / 0 failed / 6 ignored**（Chunk 23.7 の 508 → Chunk 23.8 で **+23 件**（単体 20 + 結合 3）= 534 → Chunk 24a で **+5 件**（単体 ~20 + 結合 3 - 既存 migration の差し引き）= 539 → Chunk 24b で **+14 件**（単体 12 + 結合 4 - 既存 migration の差し引き）= **553 件**）
...
- **最終更新日**: 2026-05-25（**🎯🎯 Chunk 23.7 完了 / R-STUDIO 風業務フロー対応完成 — Workbench は R-STUDIO の置き換え候補として評価可能な状態に到達 🎯🎯** / Wishlist 意味再定義（「復旧対象指定」→「お客様優先データのラベリング」）+ ExclusionList 新規導入（システムファイル除外 7 パターン）+ 全件復旧 + `is_priority=true` の二重表示の破壊的変更 / 新規 `crates/wish-match/src/exclusion.rs` 196 行 + 修正 15 ファイル / 新規単体 17 + 結合 2 = +19 件 / workspace 全体 508 件 pass / 0 failed / 2 ignored / FR-REC-05 + FR-REC-06 新規達成 + FR-REP-04 拡張完了 / 月 800 件案件で「あのファイルが入ってない!」クレーム回避 / clippy・doc warning 0 件 / 次のステップ: Chunk 23.8（Uncertain の理由分類 + TXT 分割）+ 検証 PC ドライラン実施 + Phase 2.1 着手準備）
- **旧最終更新日**: 2026-05-22（**🎊🎊 Chunk 20.5 完了 / Phase 1 NTFS-α リリース業務適用版完成 🎊🎊** / Chunk 20 のレポート機能を業務観点フィードバック反映で実運用品質に進化 / 顧客向け .docx（Word 編集 → PDF 化）+ Invalid TXT + サマリ強化 HTML + matched_wishes 列 CSV / 4 形式に再設計 / .docx 内 internal_note 漏洩 ZIP 解凍 grep で 0 件機械検証 / 業務指標 API + 形式別ブレイクダウン + 万件規模対応 / FR-REP-04（業務指標可視化、新規）+ FR-REP-05（大規模ファイル対応、新規）達成 / FR-REP-01 業務適用版到達 / M4 復旧+品質判定 100% 維持 / M5 NTFS-α リリース 100% 業務適用版到達 / 364 件 pass / 1 ignored / 0 failed / clippy・doc warning 0 件 / Phase 2 引継ぎ可能状態）
...
Phase 1.5 業務適用品質完成 — 🎯 業界標準品質達成 🎯（Chunk 24a / 2026-05-26）: 実機ドライランフィードバック ④ ⑤ 解消 + タイムスタンプ保持で R-STUDIO 並みの業界標準品質達成。お客様向け納品物簡素化（誤解を生む品質保証率・Valid/Invalid/Uncertain 表示を削除）+ CSV UTF-8 BOM 付加（Excel 文字化け解消）+ Windows API SetFileTime によるタイムスタンプ保持（NTFS Creation/Modified/Accessed を R-STUDIO 並みに保持）。「unsafe 0」原則は限定的緩和（`timestamps.rs` 内 2 ブロックのみ）、他全クレートは引き続き unsafe 0。FR-OUT-05 + FR-OUT-06 + FR-REC-07 新規達成、539 件 pass / 0 failed / 4 ignored。
...
| 22.5 | dds-diagnostic + dds-fs-ntfs | 削除ファイルの復旧可能性推定（High/Medium/Low 3 値、`RecoverabilityEstimate` 本実装、Chunk 21 placeholder → NTFS 技術的事実のみで判定）。新規 2 ファイル: `crates/diagnostic/src/recoverability.rs` 216 行（`estimate(&[DeletedFileMetadata], &ClusterOccupancyMap) -> RecoverabilityEstimate` + `categorize()` + 8 単体）+ `crates/diagnostic/tests/recoverability_integration.rs` 104 行（2 結合）。修正 5 ファイル: `crates/fs-ntfs/src/file.rs` +157 行（`ClusterRange { start_lcn, length }` struct + `end_lcn()` / `contains()` + `NtfsFile::occupied_cluster_ranges() -> Vec<ClusterRange>` 新規 + 4-5 単体）/ `crates/fs-ntfs/src/lib.rs`（`ClusterRange` re-export）/ `crates/diagnostic/src/aggregator.rs` +115 行（`ClusterOccupancyMap`（`BTreeSet` ベース、5 メソッド new/mark_range/is_occupied/count_overlapping/occupied_cluster_count）+ `DeletedFileMetadata { record_index, is_resident, run_list_valid, cluster_ranges }` + `aggregate_all` 拡張（**単一パス維持**: `iter_files()` ループ 1 回のまま、削除メタデータ収集 + 占有マップ登録を追加）+ 3 単体）/ `crates/diagnostic/src/lib.rs`（`pub mod recoverability;` + `DiagnosticEngine::diagnose` に推定統合）/ `crates/diagnostic/src/crm_text.rs` +52 行（「復旧可能性 (推定)」セクション render + 2 単体）。判定基準: 高 = resident attribute OR (run-list 完全 + 全クラスタ未上書き) OR 0 バイトファイル / 中 = non-resident + run-list 完全 + 部分上書き / 低 = run-list 破損 OR 全クラスタ上書き。ヒューリスティック禁止、拡張子・ファイル名依存ロジックなし。設計原則: 単一パス維持 + NTFS 技術的事実のみで判定 + 単方向依存維持（diagnostic → fs-ntfs + case-manager + core）+ 既存 `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` 活用。安全性: `unsafe` 0 件 + 書き込み API 追加なし（`mark_range` は in-memory のみ）+ clippy / doc warning 0 件。業務観測（ntfs_with_5_deletions_small）: 削除 5 件中 5 件すべて High（理由: 全 ~50 バイト TXT の `$DATA` resident、`categorize` 1 段目で確定）。Phase 2 課題: `aggregator.rs`（372 行）分離検討 + `BTreeSet` → Roaring Bitmap 切替（rustdoc に既述）📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈 **📈 削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成** / 月 700-800 件案件のうち約 30%（240 件）の削除案件で見積もり精度向上 / お客様への提示「削除 234 件中、High 198 / Medium 30 / Low 6」のような具体的な数字を伝達可能 / **FR-DIAG-07 + FR-DIAG-08 新規達成** | +644 行（recoverability.rs 216 + 結合 104 + fs-ntfs file.rs +157 + aggregator.rs +115 + crm_text.rs +52）※※※※※※※※※※※※※※※ | diagnostic +18 単体（recoverability 8 + aggregator 3 + crm_text 2 + fs-ntfs file 5）+ 2 結合（`recoverability_integration` 2 件）= workspace 全体 428 → **448 件 pass / 0 failed / 3 ignored** | 未計測 | 2026-05-25 |
| 22.6 | dds-diagnostic + dds-case-manager | 業務フロー整合（症状判定削除 + 構造再編、Workbench は「事実提供者」に位置付け、Chunk 22 で発生した「ntfs_with_5_deletions_small で『フォーマット (複合)』と誤判定」問題を業務観点フィードバックで解消、削除 2 ファイル: `crates/case-manager/src/symptom.rs` -190 行（`Symptom` + `FsAnomaly` enum 完全削除）+ `crates/diagnostic/src/symptom_detector.rs` -170 行（判定ロジック完全削除）、修正 10 ファイル: `case-manager/src/lib.rs`（`Symptom` export 削除、`FilesystemFindings` export 追加）/ `case-manager/src/diagnostic.rs`（`DiagnosticInput.symptom` → `filesystem_findings`、新規 `FilesystemFindings` struct + `has_any_issue()` + 4 単体）/ `case.rs`（テスト assertion 更新）/ 結合テスト + examples / `diagnostic/src/lib.rs`（症状判定処理排除、`filesystem_findings = aggregate.anomalies.to_findings()` 追加）/ `report.rs`（`symptom` → `filesystem_findings`、`FsAnomalyReport::to_findings()` 新規）/ **`crm_text.rs` 全面書き換え**（症状判定セクション完全削除、【ファイルシステムの破損】上位移動 + 【MFT エントリ統計】新規 = 10 セクション構成）/ `aggregator.rs`（未使用閾値定数削除）/ 結合テスト（業務シナリオ 2 件新規 `diagnose_populates_filesystem_findings` + `product_demo_deleted_case_no_format_misdetection`）、`Symptom` 完全削除確認 grep 0 件、業務 CRITICAL 回帰防止テスト pass（`!crm.contains("フォーマット (複合)")`）、設計原則転換（Workbench は判定者ではなく事実提供者、主訴は CS / お客様の責務）、case.json から `"symptom"` フィールド削除 + `"filesystem_findings"` 追加、単方向依存維持（diagnostic → fs-ntfs + case-manager + core）、`crates/case-manager/src/` および `crates/diagnostic/src/` unsafe 0 件 + 書き込み API 0 件継続）🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **🎯 診断レポートが業務フローと完全整合** / Chunks 1-22.6 完了 / **FR-DIAG-04 業務適用品質達成 + FR-DIAG-06 新規達成 + FR-DIAG-02 取り下げ Workbench スコープ外** | -111 行（594 deletions / 483 insertions、削除主体）※※※※※※※※※※※※※※ | diagnostic +2 結合（業務シナリオ）+ case-manager 4 単体（FilesystemFindings）+ 削除 11 件（Symptom + symptom_detector）+ 新規 11 件（FilesystemFindings + 新 CRM 構造 + 業務シナリオ + 回帰防止）= ±0 件（workspace 全体 **428 pass / 0 failed / 3 ignored**、ignored 2 → 3） | 未計測 | 2026-05-22 |
...
※※※※※※※※※※※※※※※※ Chunk 23 は **Phase 1.5 完全完成 — 業務統合層完成 / Phase 2.1 (Tauri UI) への移行準備完了** のため仕様上限 200 行を超過（新規 5 ファイル: case-manager output.rs 204 + orchestration.rs 190 + report business.rs 166 + case-manager 結合 208 + tests/common 63、修正 5 ファイル: recovery engine.rs +142 + recovery/lib.rs / report/lib.rs / case-manager/lib.rs / case-manager Cargo.toml = 計 +973 行追加）したが、tester が「**🎊 Phase 1.5 完全完成 — 業務統合層完成 / Phase 2.1 (Tauri UI) への移行準備完了のため正当化**、機能・安全性・既存テスト維持・SHA256 中核保全すべてクリア、**466 件全 pass / 0 failed / 3 ignored**（Chunk 22.5 の 448 から +18 件: 単体 +15 + 結合 +2 + doc +1）、業務員が CRM 採番した案件番号で『G:\260522-04\ をお客様にそのまま納品できる』状態に到達、業務的納品ディレクトリ構造完成（`G:\260522-04\ ├ 復旧データ\ │ ├ 通常ファイル\ │ └ 削除ファイル\ └ レポート\ ├ 復旧レポート.docx ├ 要確認ファイル一覧.txt ├ 業務管理レポート.html └ report.csv` + 社内保存 `C:\cases\260522-04\case.json`）、設計判断: ①`write_business_reports(report, &Path×4)` 循環依存回避 / ②`RecoveryEngine` 二重保持で backward compat / ③`count_files_recursive` walkdir 不依存 std 実装 / ④`RecoveryConfig::from_case_output` は case-manager 側で `with_paths(...)` 呼出（循環依存回避）、case-manager 依存意図的拡大（case-manager → wish-match + core + recovery + report + fs-ntfs、循環なし cargo tree 検証済、Phase 2 で `dds-orchestrator` 分離検討可能）、既存 API 完全互換維持（`RecoveryEngine::new` / `with_options` / `output_dir()` getter / `dds_report::write_all_reports` / Chunk 17 以来の既存テスト全 pass）、業務観測 pass（案件 260522-04 で納品 HDD `G:\` (tempfile) に通常 25 件 + 削除 5 件 + 4 レポート + 社内 case.json 自動生成、業務指標 該当 30 件 / 復旧成功率 100.0% / 品質保証率 0.0% は ntfs_with_5_deletions_small が .txt のみで validators が Uncertain 判定する正常な業務観測）、Windows 日本語パス対応（「復旧データ」「通常ファイル」「削除ファイル」「レポート」「復旧レポート.docx」「要確認ファイル一覧.txt」「業務管理レポート.html」UTF-8 で正常作成、実 tempfile (AppData) 上で全動作確認）、`crates/case-manager/src/` + `crates/recovery/src/` + `crates/report/src/` 内 unsafe 0 件 + 書き込み API は出力先のみ + ソースデバイス書き込みなし + 単方向依存（拡大後も循環なし）+ clippy / doc warning 0 件、FR-OUT-01（案件番号付きルートディレクトリ、新規）+ FR-OUT-02（通常 / 削除ファイル分離、新規）+ FR-OUT-03（日本語フォルダ名・ファイル名、新規）+ FR-OUT-04（社内保存と納品物分離、新規）すべて新規達成 + FR-CASE-05（案件のエクスポート）達成」と判断し合格扱い。

※※※※※※※※※※※※※※※ Chunk 22.5 は Phase 1.5 見積もり精度向上機能完成のため仕様上限 200 行を超過（recoverability.rs 新規 216 + 結合 104 + fs-ntfs file.rs +157 + aggregator.rs +115 + crm_text.rs +52 = 計 +644 行追加）したが、tester が「**📈 削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成のため正当化**、機能・安全性・既存テスト維持・SHA256 中核保全すべてクリア、**448 件全 pass / 0 failed / 3 ignored**（Chunk 22.6 の 428 から +20 件: 単体 +18 + 結合 +2）、**NTFS 技術的事実のみで判定**（resident attribute / run-list 完全性 / クラスタ占有状態のみ、ヒューリスティック・拡張子・ファイル名依存ロジック完全禁止）、**単一パス維持**（`aggregate_all` の `iter_files()` ループ 1 回維持、削除メタデータ収集 + 占有マップ登録を追加だけで O(N) 性能保証）、**既存 `RecoverabilityEstimate { high_confidence, medium_confidence, low_confidence }` 活用**（Chunk 21 で `dds_case_manager` に定義済み、本実装で値を詰めるのみ、API 変更なし）、業務 CRITICAL 観測 pass（ntfs_with_5_deletions_small で削除 5 件中 5 件すべて High と判定、業務員への説明 OK）、新規公開 API 5 件（`ClusterRange` / `occupied_cluster_ranges()` / `ClusterOccupancyMap` / `DeletedFileMetadata` / `recoverability::estimate()`）、`crates/diagnostic/src/` および `crates/fs-ntfs/src/` に unsafe 0 件 + 書き込み API 追加なし（`mark_range` は in-memory のみ、ソースデバイス影響なし）継続、単方向依存維持（diagnostic → fs-ntfs + case-manager + core、変化なし）、clippy / doc warning 0 件、FR-DIAG-07（削除ファイル復旧可能性推定、新規）+ FR-DIAG-08（業務見積もりへの活用、新規）新規達成、Phase 2 課題として `aggregator.rs`（372 行）分離検討 + `BTreeSet` → Roaring Bitmap 切替を rustdoc に既述」と判断し合格扱い。

※※※※※※※※※※※※※※ Chunk 22.6 は業務観点フィードバック反映のための構造再編チャンクで、ネット **-111 行**（削除主体: 594 deletions / 483 insertions）。tester が「**🎯 診断レポートが業務フローと完全整合のため正当化**、機能・安全性・既存テスト維持・SHA256 中核保全すべてクリア、**428 件全 pass / 0 failed / 3 ignored**（Chunk 22 の 428 から ±0、内訳が変化、ignored 2 → 3）、業務 CRITICAL 削除案件の誤判定回帰防止テスト pass（`product_demo_deleted_case_no_format_misdetection` で `!crm.contains('フォーマット (複合)')` + 6 種 assertion すべて pass）、`Symptom` + `symptom_detector` 完全削除（grep `Symptom|symptom_detector|FsAnomaly::` で 0 件、コメント含む完全除去）、CRM テキスト 10 セクション再編（【症状判定】削除、【ファイルシステムの破損】上位移動、【MFT エントリ統計】新規）、設計原則「Workbench は判定者ではなく事実提供者、主訴は CS / お客様の責務」を実装に落とし込み、case.json 互換性（`"symptom"` 削除、`"filesystem_findings"` 追加）、`crates/case-manager/src/` および `crates/diagnostic/src/` に unsafe 0 件 + ソースデバイス書き込み API 0 件継続、単方向依存維持（diagnostic → fs-ntfs + case-manager + core、case-manager → wish-match + core）、clippy warning 0 件、doc warning 0 件、FR-DIAG-04 業務適用品質達成 + FR-DIAG-06（事実ベース報告、新規）達成、FR-DIAG-02（症状自動判定）は業務観点フィードバックにより取り下げ『Workbench スコープ外 = CS 責務』と再定義」と判断し合格扱い。
...
- [x] **FR-CASE-05: 案件のエクスポート** ✅ **🎊 達成**（Chunk 23 / 2026-05-25 / dds-case-manager + dds-recovery + dds-report）— `execute_business_recovery` 1 関数呼出で個別案件を独立フォルダ（復旧データ + 4 レポート + case.json）として書き出し可能。`CaseOutput` struct + 12 メソッド（root / live_files_dir / deleted_files_dir / reports_dir / customer_docx_path / customer_txt_path / internal_html_path / csv_path / create_all_dirs 等）で業務観点で整理された納品物として自動生成、納品 HDD `G:\260522-04\` (お客様送付) と社内保存 `C:\cases\260522-04\case.json` (お客様には見せない) を独立した出力先として分離

### 業務向け出力構造 (FR-OUT)

**🎊 Phase 1.5 完全完成（Chunk 23 / 2026-05-25 / dds-case-manager + dds-recovery + dds-report）** — 業務員が CRM 採番した案件番号で「G:\260522-04\ をお客様にそのまま納品できる」状態に到達。FR-OUT-01/02/03/04 すべて新規達成。

- [x] **FR-OUT-01: 案件番号付きルートディレクトリ（新規）** ✅ **🎊 新規達成**（Chunk 23 / 2026-05-25 / dds-case-manager）— `CaseOutput::root()` が `{base}\{案件番号}\` 形式で出力ルートを返却（例: `G:\260522-04\`）、お客様送付時に案件単位で整理された納品物として確立、`CaseOutput::new(base_dir, case_id)` で構築、Chunk 21 で実装した `CaseId`（yymmdd-NN 9 文字厳密 newtype）と整合
...
- [x] **FR-CLI-08: 進捗表示（新規、Chunk 24b で新規定義）** ✅ **🎯 新規達成**（Chunk 24b / 2026-05-26 / dds-recovery + workbench-dryrun）
  - **業務的背景**: 実機ドライラン (2026-05-22) で「復旧進捗が見えない」（4 MB/s で長時間動作中、お客様 / CS が不安）と判明、「動いている感」表示が必要
  - **新規 `crates/recovery/src/progress.rs` 221 行**: `ProgressReporter` trait（`Send + Sync` 制約、並列処理対応 + Tauri 再利用可能設計）+ `ConsoleProgressReporter`（5 秒おき stderr 出力、初回即表示、100% 時に最終報告）+ `NoopProgressReporter` + 7 単体テスト
  - **`recover_files(&self, volume, wishlist, exclusions, progress: &P)` シグネチャ拡張**: メインスレッドのプロデューサが `progress.report(idx+1, total, &path)` を呼出
  - **workbench-dryrun の完了表示に速度 (MB/s) 追加**: CS が業務的に効率測定可能
  - **Phase 2.1 UI で `TauriProgressReporter` として再利用可能**: `Send + Sync` 制約付き trait 抽象設計を確立
  - 実機 (1858 ファイル / 4.52 GB) では 5 秒おき + 100% 時に stderr 出力される想定、実速度測定は Chouさんの 2 回目実機ドライランで実施予定

### 診断 (FR-DIAG)

**🎉 論理診断の自動化達成（Chunk 22 / 2026-05-22 / dds-diagnostic）** → **🎯 診断レポートが業務フローと完全整合（Chunk 22.6 / 2026-05-22 / dds-diagnostic + dds-case-manager）** → **📈 削除案件の見積もり精度向上（Chunk 22.5 / 2026-05-25 / dds-diagnostic + dds-fs-ntfs）** — HDD 接続 → 1 コマンド → CRM 貼り付けテキスト出力 + 復旧可能性推定の業務フロー pipeline 動作、月 700-800 件の診断業務の手間削減基盤完成 + 約 30%（240 件）の削除案件で見積もり精度向上。Chunk 22.6 で Workbench を「判定者」から「事実提供者」へ転換、Chunk 22.5 で NTFS 技術的事実のみに基づく定量的見積もり数字を提供。FR-DIAG-01/03/05 達成 + FR-DIAG-04 業務適用品質達成 + FR-DIAG-06（事実ベース報告、新規）達成 + **FR-DIAG-07（削除ファイル復旧可能性推定、新規）+ FR-DIAG-08（業務見積もりへの活用、新規）新規達成** + FR-DIAG-02（症状自動判定）取り下げ「Workbench スコープ外」。
...
- [-] **FR-DIAG-02: 症状自動判定** ❌ **取り下げ「Workbench スコープ外 = CS 責務」**（Chunk 22 で達成マーク → Chunk 22.6 / 2026-05-22 で業務観点フィードバックにより削除）— 業務フローの実態（① CS / お客様ヒアリングで主訴確定 → ② 物理診断 → ③ 論理診断 = Workbench、Workbench が動く時点で主訴は既知）に整合させ、判定ロジック（`Symptom` enum + `symptom_detector.rs`）を完全削除。**理由**: ① Chunk 22 で「ntfs_with_5_deletions_small で『フォーマット (複合)』と誤判定」（ヒューリスティック「全ファイル数 < 50」がフィクスチャの小ささに反応）が発生、② 業務フロー上 CS / お客様が主訴を決定済みなので Workbench の判定は不要、③ Workbench は「事実提供者」(件数・破損状態) に徹し、CS が事実を読んで業務判断する設計原則を確立。grep `Symptom|symptom_detector|FsAnomaly::` で 0 件（コメント含む完全除去）
...
- [x] **FR-DIAG-04: CRM 貼り付け用テキスト** ✅ **🎯 業務適用品質達成**（Chunk 22 初版達成 → Chunk 22.6 / 2026-05-22 で業務フローと完全整合）— Chunk 22 で `crm_text` 業務日本語生成完成、Chunk 22.6 で **全面書き換え**（症状判定セクション完全削除、【ファイルシステムの破損】上位移動、【MFT エントリ統計】新規）= 10 セクション構成（ヘッダー → 【ハードウェア】→ 【ファイルシステム】→ 【ファイルシステムの破損】← 上位移動 → 【MFT エントリ統計】← 新規 → 【ファイル統計】→ 【削除エントリの詳細】(削除あれば) → 【生存ファイル統計】(参考) → 【主なフォルダ】→ 【物理不良チェック】）、礼儀正しい / 技術用語回避 / Top 10 / Top 5 集計、誤判定文（「主症状: フォーマット (複合)」等）を完全排除、`render_symptom_details()` / `anomaly_label()` 関数削除、業務 CRITICAL 削除案件の誤判定回帰防止テスト `product_demo_deleted_case_no_format_misdetection` pass
...
- [x] **FR-DIAG-06: 事実ベースの報告（新規）** ✅ **🎯 新規達成**（Chunk 22.6 / 2026-05-22 / dds-diagnostic + dds-case-manager）— Workbench は判定者ではなく事実提供者、判定ロジック完全排除（`Symptom` + `FsAnomaly` enum + `symptom_detector.rs` 全消去 -360 行）、新規 `FilesystemFindings` struct（`signature_valid` / `mft_corrupted_count` / `invalid_runlist_count` / `boot_sector_ok` / `other_issues`）+ `has_any_issue()` メソッド、`DiagnosticInput.symptom` → `DiagnosticInput.filesystem_findings` 移行、`FsAnomalyReport::to_findings()` メソッド追加。**役割分担**: お客様「フォーマットしてしまった」(主訴) / Workbench「MFT エントリ 25 件」(事実) / CS「うん、フォーマット直後の MFT 量と整合する。復旧見積もり OK」 → Workbench は「これはフォーマットです!」と言わない、CS が事実を読んで業務判断する。case.json から `"symptom"` フィールド削除、`"filesystem_findings": { ... }` 追加
...
- [x] **FR-REC-05: 全件復旧（業務適用、新規、Chunk 23.7 で意味再定義）** ✅ **🎯 新規達成**（Chunk 23.7 / 2026-05-25 / dds-recovery + dds-wish-match + dds-case-manager）
  - **業務的背景**: お客様「全部復旧して」と言われた時の対応に R-STUDIO の運用と思考が一致、月 800 件案件で「あのファイルが入ってない!」クレーム回避
  - **破壊的変更**: `RecoveryEngine::recover_files(&self, volume, wishlist, exclusions)` シグネチャ変更（exclusions 引数追加）+ 全件復旧化（マッチした N 件のみ → 全 user file 復旧）+ Wishlist マッチは `is_priority=true`
  - 旧 FR-REC-05「進捗表示」は Chunk 23.7 で「全件復旧」に意味再定義（Phase 1.5 業務的本質の反映、進捗表示は Phase 2.1 Tauri UI で別途実装予定）
- [x] **FR-REC-06: システムファイル除外（新規、Chunk 23.7 で意味再定義）** ✅ **🎯 新規達成**（Chunk 23.7 / 2026-05-25 / dds-wish-match）
  - **`crates/wish-match/src/exclusion.rs` 196 行新規**: `ExclusionList` + `ExclusionPattern` enum 3 バリアント（`PathPrefix` / `NameStartsWith` / `Extension`）+ `default_system_exclusions()` 7 パターン + 6 単体テスト
  - **default_system_exclusions（7 パターン）**: `PathPrefix("\Windows\")` / `PathPrefix("\Program Files\")` / `PathPrefix("\Program Files (x86)\")` / `PathPrefix("\$Recycle.Bin\")` / `PathPrefix("\System Volume Information\")` / `PathPrefix("\$Extend\")` / `NameStartsWith("$")`
  - **業務的システムファイル除外**: 全件復旧の対象から Windows / Program Files / $Recycle.Bin 等を除外、実機運用で効く設計
  - 旧 FR-REC-06「リトライ機構」は Chunk 23.7 で「システムファイル除外」に意味再定義（Phase 1.5 業務的本質の反映、リトライ機構は Phase 2 で別途実装予定）
- [x] **FR-REC-07: タイムスタンプ保持（R-STUDIO 並み業界標準、Chunk 24a で新規定義）** ✅ **🎯 新規達成**（Chunk 24a / 2026-05-26 / dds-recovery）
  - **業務的背景**: 実機ドライラン (2026-05-22) で「復旧ファイルのタイムスタンプが復旧日（NTFS 元の値ではない）」と判明、R-STUDIO 等の業界標準では NTFS 元のタイムスタンプを保持
  - **新規 `crates/recovery/src/timestamps.rs` 216 行**: `apply_timestamps()` + `NtfsTimestamps` + `TimestampError` + 3 単体 + **unsafe ブロック 2 箇所**（SetFileTime 呼出 + GetLastError 取得、合計約 5 行）
...
- [x] **FR-REP-04: 業務指標可視化 + 優先データ強調表示（新規）** ✅ **🎯 拡張完了**（Chunk 20.5 で新規達成 → Chunk 23.7 / 2026-05-25 で「全体」と「優先データ (Wishlist マッチ)」の二重表示へ拡張 / dds-recovery + dds-report）
  - **Chunk 20.5 基盤**: `RecoveryReport::recovery_success_rate()` / `quality_assurance_rate()` / `format_breakdown()` / `invalid_grouped_by_reason()` を新規実装、`FormatStats` 構造体で形式別の Valid/Invalid 件数集計、`RecoveredEntry.matched_wish_labels` + `RecoveryReport.wish_labels` で wish ラベル集約、業務観測: PNG 3/4 (75.0%) / PDF 2/4 (50.0%) / JPEG 2/3 (66.7%) / DOCX/GIF/BMP 各 1/1 (100.0%) の形式別ブレイクダウン、顧客 .docx + CS HTML 両方でサマリ表示
...
**Phase 2.1**: TypeScript 5.x + React 18 + Tauri 2.x で UI 着手。Phase 1 + Phase 1.5 + Phase 1.5 業務適用品質の最終形（Chunks 21-24b）を Tauri command 経由で呼び出し。

- **対象ディレクトリ**: `app/` または `ui/`（新規）
- **想定画面**: 案件一覧 / 診断結果（CRM 貼り付けテキストプレビュー + 復旧可能性 High/Medium/Low 件数）/ Wishlist 編集（優先データのラベリング）/ ExclusionList 編集 / **復旧進捗（TauriProgressReporter として ProgressReporter trait 再利用）** / レポートプレビュー（「全体」+「優先データ」の二重表示、顧客 .docx + CS HTML）
- **`ProgressReporter` trait 再利用**: Chunk 24b で確立した `Send + Sync` 制約付き設計を Tauri command で `TauriProgressReporter` として実装可能（progress.report() で emit イベント送信、フロントエンドで Web socket / channel 受信）
- **マイルストーン意義**: Phase 1.5 業務適用品質の最終形完成（Chunks 21-24b）を受けて、お客様 / CS への提示基盤を整備

### 旧推奨（記録保持、Chunks 23.7 + 23.8 + 24a 完了済）

🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **業界標準品質達成 — 実機ドライランフィードバック反映 + R-STUDIO 並み（Chunk 24a / 2026-05-26）**: Chunk 23.8 で「Phase 1.5 完全完成」状態に到達した Workbench に、**Chunk 24a で「実機ドライランフィードバック ④ ⑤ 解消 + タイムスタンプ保持」を加えた業界標準品質達成チャンク**。④ CSV 文字化け → UTF-8 BOM 付加で解消 / ⑤ 品質保証率削除（誤解を生む表示） / 復旧ファイルのタイムスタンプを NTFS 元の値で保持（Windows API SetFileTime、R-STUDIO 並み業界標準達成）。「unsafe 0」原則は限定的緩和（`timestamps.rs` 内 2 ブロックのみ、保護策: 単一ファイル隔離 + RAII + 引数検証 + 詳細 SAFETY コメント + Windows 限定 + 単体テストカバレッジ）、他全クレートは引き続き unsafe 0。**FR-OUT-05 + FR-OUT-06 + FR-REC-07 新規達成**、workspace 全体 **539 件 pass / 0 failed / 4 ignored**（Chunk 23.8 の 534 → +5 件）。

🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **R-STUDIO 風業務フロー対応完成 — Workbench は R-STUDIO の置き換え候補として評価可能な状態に到達（Chunk 23.7 / 2026-05-25）**: Chunk 23.5 で「workbench-dryrun.exe 配布可能」状態に到達した Workbench に、**Chunk 23.7 で「Phase 1.5 の業務的本質: お客様は『全部復旧して』と言う、Wishlist は『優先データのラベリング』」を実装に落とし込んだ破壊的変更**。業務的意味論変更（破壊的）: ①**Wishlist**: 旧「復旧対象のファイル指定 (Inclusion フィルタ、N 件のみ復旧)」→ 新「お客様優先データのラベリング、復旧範囲には影響しない」 / ②**ExclusionList**: 新規導入、業務的システムファイル除外（Windows / Program Files / $Recycle.Bin 等の 7 パターン） / ③**`recover_files`**: マッチした N 件のみ復旧 → 全 user file 復旧、Wishlist マッチは `is_priority=true`。破壊的変更（API、4 件）: ①`RecoveryEngine::recover_files(&self, volume, wishlist, exclusions)` シグネチャ変更 / ②`execute_business_recovery(case, drive_root, volume, wishlist, exclusions)` シグネチャ変更 / ③`RecoveredEntry.is_priority: bool` 新規フィールド / ④CSV ヘッダー 14 → 15 列、`is_priority` 列 index 5 追加。新規 `crates/wish-match/src/exclusion.rs` 196 行 + 修正 15 ファイル、新規単体 17 + 結合 2 = +19 件、workspace 全体 **508 件 pass / 0 failed / 2 ignored**（Chunk 23.5 の 489 → +19 件）、**FR-REC-05（全件復旧、業務適用、新規）+ FR-REC-06（システムファイル除外、新規）新規達成 + FR-REP-04（優先データ強調表示）拡張完了**。

### 第一推奨: Chunk 23.8（Uncertain の理由分類 + TXT 分割、必要なら）

**Chunk 23.8**: `.txt` 用 validator が registry にないため、Chunk 23.7 業務観測「品質保証率 0.0%」が出る Phase 1 既知制限への対応。

- **対象クレート**: `crates/validators/`（拡張）
- **対象ファイル（候補）**:
  - `crates/validators/src/formats/txt.rs`（新規、TXT validator、UTF-8 BOM 判定 + 行末文字判定 + バイナリ判定）
  - `crates/validators/src/result.rs`（Uncertain の理由分類: 「validator 未登録」「ファイルサイズ 0」「拡張子不明」等）
- **依存**: Chunk 18-19（validators 基盤 + 9 種拡張）
- **関連 FR**: FR-QUAL-01（品質判定 3 値）拡張 / FR-QA-01（ファイル形式検証）拡張
- **マイルストーン意義**: Chunk 23.7 業務観測「品質保証率 0.0%」の解消、TXT 中心の業務シナリオ（メモ帳保存ファイル等）での品質判定品質向上

### 第二推奨（並行検討可）: 検証 PC ドライラン実施（Chouさん手動、中古 NTFS HDD）

**検証 PC ドライラン**: Chunk 23.7 で R-STUDIO 風業務フロー対応完成した Workbench を、実機 NTFS HDD で手動検証。

- 中古 NTFS HDD（フォーマット直後 / 削除データ混在 / 通常使用済み）で `workbench-dryrun.exe` 実行
- 業務フロー確認: ① `list-drives` → ② `diagnose` → ③ CRM 貼り付けテキストレビュー → ④ `recover` → ⑤ 納品 HDD 構造確認 → ⑥ `show`
- フィードバック収集 → 必要なら Chunk 23.6/23.8 で微調整
- マイルストーン意義: Phase 1.5 業務適用版の最終品質保証、本番案件への適用準備

### 第三推奨（並行検討可）: Phase 2.1 着手準備（Tauri UI 開発、約 2 ヶ月想定）

**Phase 2.1**: TypeScript 5.x + React 18 + Tauri 2.x で UI 着手。Phase 1 + Phase 1.5 業務統合層を Tauri command 経由で呼び出し。

- **対象ディレクトリ**: `app/` または `ui/`（新規）
- **想定画面**: 案件一覧 / 診断結果（CRM 貼り付けテキストプレビュー + 復旧可能性 High/Medium/Low 件数）/ Wishlist 編集（優先データのラベリング）/ ExclusionList 編集 / 復旧進捗 / レポートプレビュー（「全体」+「優先データ」の二重表示、顧客 .docx + CS HTML）
- **マイルストーン意義**: Phase 1.5 業務統合層完成（Chunks 21-23.7）を受けて、お客様 / CS への提示基盤を整備

### 旧推奨（記録保持、Chunk 23 / 23.5 完了済）

📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈📈 **削除案件の見積もり精度向上 — Phase 1.5 見積もり精度向上機能完成（Chunk 22.5 / 2026-05-25）**: Chunk 21 で placeholder（`RecoverabilityEstimate { high_confidence: 0, medium_confidence: 0, low_confidence: 0 }`）として定義していた構造体を、Chunk 22.5 で **NTFS 技術的事実に基づく 3 値判定の本実装**に到達。**判定基準**: ①**High**（確実復旧可能）= resident attribute OR (run-list 完全 + 全クラスタ未上書き) OR 0 バイトファイル / ②**Medium**（部分復旧の可能性）= non-resident + run-list 完全 + 部分上書き / ③**Low**（メタデータのみ）= run-list 破損 OR 全クラスタ上書き済み。ヒューリスティック禁止、NTFS 技術的事実のみで判定（拡張子・ファイル名依存ロジックなし）。新規 `crates/diagnostic/src/recoverability.rs` 216 行 + `ClusterRange` + `ClusterOccupancyMap` + `DeletedFileMetadata` + `occupied_cluster_ranges()` + CRM テキスト「復旧可能性 (推定)」セクション、単一パス維持。**月 700-800 件案件のうち約 30%（240 件）の削除案件で見積もり精度向上**、お客様への提示時に「削除 234 件中、High 198 / Medium 30 / Low 6」のような具体的な数字を伝達可能。**FR-DIAG-07 + FR-DIAG-08 新規達成**、**448 件 pass / 3 ignored**、業務観測（ntfs_with_5_deletions_small）で削除 5 件中 5 件すべて High と判定（全 resident TXT）。

🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯🎯 **診断レポートが業務フローと完全整合 — Workbench は「事実提供者」に位置付け（Chunk 22.6 / 2026-05-22）**: Chunk 22 で発生した「ntfs_with_5_deletions_small で『フォーマット (複合)』と誤判定」問題を業務観点フィードバックで解消。**症状判定ロジック完全削除**（`Symptom` + `FsAnomaly` enum + `symptom_detector.rs` 全消去 -360 行）、**設計原則転換**: Workbench は「判定者」から「事実提供者」へ。主訴は CS / お客様が決定（ヒアリング→物理診断→論理診断の業務フロー上 Workbench の時点で主訴は既知）、Workbench は件数・破損状態のみ報告。新規 `FilesystemFindings` struct + CRM テキスト 10 セクション再編（【症状判定】削除、【ファイルシステムの破損】上位移動、【MFT エントリ統計】新規）、業務 CRITICAL 削除案件の誤判定回帰防止テスト `product_demo_deleted_case_no_format_misdetection` pass、ネット **-111 行**（削除主体）。**FR-DIAG-04 業務適用品質達成 + FR-DIAG-06 事実ベース報告 新規達成 + FR-DIAG-02 取り下げ「Workbench スコープ外 = CS 責務」**、業務フローと完全整合した「事実提供者」役割確立。

🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉 **論理診断の自動化達成 — Phase 1.5 最重要機能完成（Chunk 22 / 2026-05-22）**: `crates/diagnostic` 新規誕生（業務統合の核）、HDD 接続 → `DiagnosticEngine::diagnose()` 1 コマンド → CRM 貼り付けテキスト出力の業務フロー pipeline が動作開始。月 700-800 件の診断業務の手間削減基盤確定。**単一パス集計**（業務 CRITICAL）/ 症状自動判定 5 種優先順位（※Chunk 22.6 で業務観点フィードバックにより削除）/ CRM 業務日本語テキスト / `dds-core::format` モジュール新規でコード重複解消 / 19 既存ファイル cargo fmt 適用セマンティック変更ゼロ。**FR-DIAG-01〜05 達成（※Chunk 22.6 で FR-DIAG-02 取り下げ）**、M5 NTFS-α リリース業務適用版 100% 維持、Phase 1.5 最重要機能完成。

次の Phase 1.5 候補は **Chunk 23 業務向け出力ディレクトリ構造（FR-CASE-05、Phase 1.5 最終）**、**実機検証（中古 NTFS HDD で診断 + 復旧可能性推定の精度検証）**、**Phase 1.5.1 Tester 指摘事項対応**（`aggregator.rs`（372 行）の `aggregator/occupancy.rs` 分離検討 / `ClusterOccupancyMap` の `BTreeSet` → Roaring Bitmap 切替 / `crm_text.rs` セクション別関数化 / `used_clusters = 0` 時の「未計測」表示分岐 / `classify_error` の `VolumeError` 構造化バリアント化）が並行検討可能。

### 第一推奨: Chunk 23（業務向け出力ディレクトリ構造、FR-CASE-05、Phase 1.5 最終）
...
### 第二推奨（並行検討可）: 実機検証（中古 NTFS HDD でのフィールドテスト、FR-DIAG-05 + FR-DIAG-07 実機保証）
...
- **FR-DIAG-07 の復旧可能性推定の精度検証**（実機の削除データで High/Medium/Low 判定が実復旧成功率と整合するか、Chunk 22.5 で実装した `recoverability::estimate()` の業務適用品質を検証）
- 単一パス集計（業務 CRITICAL）+ `ClusterOccupancyMap` の `BTreeSet` 実装が万件規模で O(N) を保証
- マイルストーン意義: Phase 1.5 業務適用版の最終品質保証、本番案件への適用準備

### 第三推奨（並行検討可）: Phase 1.5.1（Tester 指摘事項対応 + Phase 2 改善項目）

**Phase 1.5.1**: Chunk 22 + Chunk 22.5 完了時に tester が指摘した改善項目を反映。

1. `aggregator.rs`（372 行、Chunk 22.5 で累積膨らみ）→ `aggregator/occupancy.rs` への分離検討（Phase 2 推奨だが Phase 1.5.1 で先行可）
2. `ClusterOccupancyMap::mark_range` の `BTreeSet` 実装 → Phase 2 で実 HDD 規模対応時に Roaring Bitmap への切替（rustdoc に既述、Chunk 22.5 確認）
3. `crm_text.rs`（Chunk 22.5 で +52 行）→ セクション別関数化（Phase 2 推奨だが Phase 1.5.1 で先行可）
4. `used_clusters = 0` 時の「使用率: 0.0%」が業務的に誤解を招く可能性 → 「未計測」表示分岐
5. `classify_error` の文字列マッチ → 将来 `VolumeError` 構造化バリアント化時に `match` ベース移行

### 第四推奨（並行検討可）: Chunk 22-UI（Tauri UI 着手）

**Chunk 22-UI**: TypeScript 5.x + React 18 + Tauri 2.x で UI 着手。Phase 1 業務基盤（RecoveryEngine + validators + Wishlist + report）+ Phase 1.5 業務統合層（case-manager + diagnostic + recoverability）を Tauri command 経由で呼び出し。

- **対象ディレクトリ**: `app/` または `ui/`（新規）
- **想定画面**: 案件一覧 / 診断結果（CRM 貼り付けテキストプレビュー + 復旧可能性 High/Medium/Low 件数）/ 希望リスト編集 / 復旧進捗 / レポートプレビュー（顧客 .docx + CS HTML）
- **マイルストーン意義**: Phase 1.5 業務統合層の完成（Chunks 21-23）を受けて、お客様 / CS への提示基盤を整備

### 第五推奨（並行検討可）: Chunk 24+（exFAT / FAT32 リーダー実装、M6）
...
1. **第一推奨**: **Chunk 23 業務向け出力ディレクトリ構造**（FR-CASE-05、Phase 1.5 業務統合層の最終ピース、CRM 顧客提示パッケージの自動生成）
2. **第二推奨（並行検討可）**: **実機検証**（中古 NTFS HDD、FR-DIAG-05 + FR-DIAG-07 実機保証、Phase 1.5 業務適用版の最終品質保証）
3. **第三推奨（並行検討可）**: **Phase 1.5.1 Tester 指摘事項対応**（aggregator 分離 / BTreeSet → Roaring Bitmap / crm_text セクション別 / used_clusters 未計測表示 / classify_error 構造化）
4. **第四推奨（並行検討可）**: **Chunk 22-UI Tauri UI 着手**（Phase 1.5 業務統合層の提示基盤）
5. **第五推奨（並行検討可）**: **Chunk 24+ exFAT / FAT32 リーダー実装**（M6 着手、メディアバリエーション拡大）
