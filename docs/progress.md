...
- **総単体テスト数**: 約 **425**（Chunk 22.5 までの 370 + Chunk 23 で +15 = 385 + Chunk 23.5 で workbench-dryrun +23 = 408 + Chunk 23.7 で **+17 件**（ExclusionList 6 + recover_files 3 + RecoveryReport priority 3 + CSV 1 + HTML 2 + DOCX 2）= 約 **425**）
- **総結合テスト数**: **74**（全パス + 2 ignored、Chunk 22.5 までの 70 + Chunk 23 で +2 = 72 + Chunk 23.5 で +0（実機テストで代替）= 72 + Chunk 23.7 で **+2 件**（`full_business_flow_recovers_all_files_with_priority` + `product_demo_phase_1_5_business_aligned`）= **74 件**）
...
- **総テスト数（workspace 全体）**: **508 件 pass / 0 failed / 2 ignored**（Chunk 23 の 466 → Chunk 23.5 で **+23 件**（workbench-dryrun 単体 23）= 489 → Chunk 23.7 で **+19 件**（単体 +17（ExclusionList 6 + recover_files 3 + RecoveryReport priority 3 + CSV 1 + HTML 2 + DOCX 2）+ 結合 +2（`full_business_flow_recovers_all_files_with_priority` + `product_demo_phase_1_5_business_aligned`））= **508 件**）
